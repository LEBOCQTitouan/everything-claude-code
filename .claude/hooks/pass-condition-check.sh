...
if [ ! -f "$IMPL_FILE" ]; then
  echo "WARNING: implement-done.md not found." >&2
  exit 0
fi

# Check for ## Pass Condition Results heading
if ! grep -q '^## Pass Condition Results' "$IMPL_FILE"; then
  echo "WARNING: implement-done.md missing '## Pass Condition Results' section." >&2
  exit 0
fi

# Warn on any failures
if grep -q '❌' "$IMPL_FILE"; then
  echo "WARNING: Pass condition failures found in implement-done.md:" >&2
  grep '❌' "$IMPL_FILE" >&2
  echo "" >&2
...
if ! grep -qE 'All pass conditions:.*✅' "$IMPL_FILE"; then
  echo "WARNING: Missing 'All pass conditions: ... ✅' summary line." >&2
  ...
  while IFS= read -r test_file; do
    ...
    if echo "$test_file" | grep -qE '\.rs$'; then
      ...
      ' "$FULL_PATH" 2>/dev/null; then
        echo "WARNING: E2E test without #[ignore] in $test_file" >&2
        echo "All E2E/integration tests should have #[ignore] by default." >&2
