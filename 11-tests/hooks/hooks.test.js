...
async function runTests() {
  ...
  if (
    test('all hook commands use node or are skill shell scripts', () => {
      const hooksPath = path.join(__dirname, '..', '..', '07-hooks', 'hooks.json');
      const hooks = JSON.parse(fs.readFileSync(hooksPath, 'utf8'));
        ...
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            if (hook.type === 'command') {
              ...
              const isBashRunner = /^(bash|sh)\s/.test(hook.command) && (hook.command.includes('/skills/') || hook.command.includes('/10-scripts/') || hook.command.includes('/05-skills/'));
  ...
  if (
    test('hooks.json exists in 07-hooks/', () => {
      const hooksPath = path.join(__dirname, '..', '..', '07-hooks', 'hooks.json');
  ...
  const realUtilsPath = path.resolve(__dirname, '..', '..', '10-scripts', 'lib', 'utils.js');
...
`;
  ...
  if (
    await asyncTest('readStdinJson resolves with {} when stdin never closes (timeout fires, no data)', async () => {
      ...
      const script = 'const u=require("./10-scripts/lib/utils");u.readStdinJson({timeoutMs:100}).then(d=>{process.stdout.write(JSON.stringify(d));process.exit(0)})';
      return new Promise((resolve, reject) => {
