...
// ---------------------------------------------------------------------------
const COMMANDS = ['install', 'init', 'help', 'version', 'update', 'completion', 'audit'];
...
try {
  ...
  completion.on('arg', ({ before, reply }) => {
    switch (before) {
      ...
      case 'init':
        return reply(['--template', ...LANGUAGES]);
      case 'help':
        return reply(COMMANDS);
      case 'completion':
        return reply(['bash', 'zsh', 'fish', 'pwsh']);
      default:
        return reply([]);
    }
  });
...
if (cmd === 'completion-server') {
  try {
    ...
    const matches = current ? candidates.filter(c => c.startsWith(current)) : candidates;
...
// ---------------------------------------------------------------------------
// `ecc audit` — run configuration audit
// ---------------------------------------------------------------------------
if (cmd === 'audit') {
  const auditScript = path.resolve(__dirname, '..', 'dist', 'audit.js');
  const r = spawnSync('node', [auditScript, ...args.slice(1)], {
    stdio: 'inherit',
    env: process.env
  });
  process.exit(r.status ?? 0);
