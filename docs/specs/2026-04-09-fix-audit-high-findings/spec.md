...
|---|----------|--------|--------|
| 0 | Root cause | Systemic — rapid growth outpaced hygiene discipline | User |
| 1 | Root cause vs symptom | Systemic — 6 findings are symptoms of velocity-vs-quality imbalance | User |
| 2 | Minimal vs proper fix | Full structural — all 7 files decomposed, full clock port, full regex migration | User |
| 3 | Missing tests | Comprehensive — test every change: prune boundary, regex, warn emission, re-exports | User |
| 4 | Regression risk | Re-export breakage from file splits is primary concern | Recommended |
| 5 | Related audit findings | Bundle overlapping MEDIUMs — CORR-002/003, ERR-002/003/004, CONV-003, DOC-002/003 | Recommended |
| 6 | Reproducibility | Sufficient — all findings reproducible via cargo test, clippy, wc, grep | Recommended |
| 7 | Data impact | Verify with equivalence test — no schema migration needed | User |

### User Stories

| ID | Title | AC Count | Dependencies |
|----|-------|----------|--------------|
| US-001 | Fix Failing Prune Test | 5 | none |
| US-002 | Decompose Oversized Files | 7 | none |
| US-003 | Standardize Regex on LazyLock | 5 | none |
| US-004 | Increase Doc-Comment Coverage | 4 | US-002 |
| US-005 | Convert Swallowed Errors to Logged Warnings | 6 | US-002 |

### Acceptance Criteria

| AC ID | Description | Source US |
|-------|-------------|----------|
| AC-001.1 | Prune cutoff computed in Rust via injected Clock, bound SQL parameter | US-001 |
| AC-001.2 | Clock-injected test: prune(1) deletes old, keeps recent | US-001 |
| AC-001.3 | Equivalence test: old/new prune produce identical results | US-001 |
| AC-001.4 | No format! string interpolation in prune SQL | US-001 |
| AC-001.5 | BypassStore trait signature unchanged | US-001 |
| AC-002.1 | patterns.rs decomposed into 5 submodules, each <800 lines | US-002 |
| AC-002.2 | tests_helpers.rs decomposed into 3 submodules | US-002 |
| AC-002.3 | orchestrator_tests.rs decomposed into 3 submodules | US-002 |
| AC-002.4 | sqlite_memory.rs decomposed into 3 submodules | US-002 |
| AC-002.5 | hook/mod.rs decomposed into 3 submodules | US-002 |
| AC-002.6 | merge.rs decomposed into 2 files | US-002 |
| AC-002.7 | All public items re-exported at original path | US-002 |
| AC-003.1 | 15 inline Regex::new() converted to LazyLock | US-003 |
| AC-003.2 | 4 OnceLock converted to LazyLock | US-003 |
| AC-003.3 | clippy allow suppression removed, is_multiple_of used | US-003 |
| AC-003.4 | 5 duplicate is_leap extracted to ecc-domain | US-003 |
| AC-003.5 | LazyLock panics on first access with descriptive message | US-003 |
| AC-004.1 | ecc-domain doc coverage >50% | US-004 |
| AC-004.2 | ecc-app doc coverage >30% | US-004 |
| AC-004.3 | ecc-infra doc coverage >30% | US-004 |
| AC-004.4 | All port traits have doc comments | US-004 |
| AC-005.1 | 15 production let _ = converted to tracing::warn | US-005 |
| AC-005.2 | Bypass audit recording warns on failure | US-005 |
| AC-005.3 | Drift check file ops warn on failure | US-005 |
| AC-005.4 | Session merge recovery warns on failure | US-005 |
| AC-005.5 | Merge rebase abort warns on failure | US-005 |
| AC-005.6 | Fire-and-forget metrics documented with comment | US-005 |

### Adversary Findings

| Dimension | Score | Verdict | Key Rationale |
|-----------|-------|---------|---------------|
| Ambiguity | 75 | PASS | Concrete submodule names, clear scope |
| Edge Cases | 72 | PASS | Factual errors corrected (phase_gate, OnceLock count) |
| Scope | 85 | PASS | Well-bounded with explicit non-requirements |
| Dependencies | 80 | PASS | Clock constructor injection resolves trait contradiction |
| Testability | 82 | PASS | Every AC has a verifiable command |
| Decisions | 78 | PASS | 10 decisions cover all key trade-offs |
| Rollback | 75 | PASS | File splits reversible, regex conversion mechanical |
