...
| 3 | File scope: CLAUDE.md only or CLAUDE.md + AGENTS.md? | CLAUDE.md + AGENTS.md from v1. Forward-compat with ADR 0062 | User |
| 4 | Archived status: resolved or unresolved? | Presence-only: archived = resolved. Simpler logic, no frontmatter parsing | Recommended |
| 5 | Markdown parser: hand-rolled or pulldown-cmark? | Hand-rolled fence skip mirroring `extract_claims`. Zero new deps | Recommended |
| 6 | BL-150 stale marker fix: included in spec or separate PR? | Include in spec. Ship lint + fix known drift atomically | Recommended |
| 7 | CI wiring: `--strict` from day 1 vs WARN-then-promote vs CLI-only? | Wire with `--strict` from day 1 in ci.yml validate job | Recommended |
| 8 | ADR: file one for docs↔backlog ACL pattern? | No ADR. Capture in commit messages + CHANGELOG | Recommended |
| 9 | Scope boundary: out-of-scope items? | All four excluded from v1: (a) orphaned-entry inverse lint, (b) arbitrary markdown paths, (c) --fix auto-removal, (d) doc-validator integration | Recommended |

### User Stories

| ID | Title | AC Count | Dependencies |
|----|-------|----------|--------------|
| US-001 | Detect TEMPORARY markers with missing backlog files | 14 | none |
| US-002 | `--strict` escalates warnings to errors | 5 | US-001 |
| US-003 | Multi-file CLAUDE.md + AGENTS.md scan | 7 | US-001 |
| US-004 | One-time repo audit report | 4 | US-001, US-003 |
| US-005 | Remove stale CLAUDE.md:108 TEMPORARY (BL-150) line | 3 | US-001, US-002 |
| US-006 | CI wiring | 6 | US-001, US-002, US-005 |

### Acceptance Criteria (selected high-signal)

| AC ID | Description | Source US |
|-------|-------------|-----------|
| AC-001.1 | Real BL (BL-156) on disk → no diagnostic | US-001 |
| AC-001.2 | Missing BL (BL-999) → WARN stderr with remediation guidance | US-001 |
| AC-001.3 | Regex `(?i)TEMPORARY\s*\(BL-0*(\d{1,6})\)` with mixed-case + 6-digit variants | US-001 |
| AC-001.10 | Baseline: zero markers → zero diagnostics, exit 0 | US-001 |
| AC-001.11 | Missing `docs/backlog/` dir → all markers reported as missing | US-001 |
| AC-001.13 | `--counts` legacy flag emits deprecation warning to stderr | US-001 |
| AC-002.2 | `--strict` flips WARN→ERROR prefix, exit code 1 | US-002 |
| AC-003.7 | Walker hand-rolled on `FileSystem::read_dir`, depth 64 cap | US-003 |
| AC-004.1 | Audit table: every marker = one row, status resolved iff file exists | US-004 |
| AC-005.2 | Post-fix `ecc validate claude-md markers --strict` → exit 0 | US-005 |
| AC-006.4 | Rollback = single atomic ci.yml commit revert | US-006 |
| AC-006.5 | Kill switch `ECC_CLAUDE_MD_MARKERS_DISABLED=1` exits 0 | US-006 |
| AC-006.6 | Refactor regression guard: existing `ecc backlog` tests unchanged | US-006 |

### Adversary Findings (Round 2 — PASS)

| Dimension | Round 1 | Round 2 | Verdict | Key Rationale |
|-----------|---------|---------|---------|---------------|
| Ambiguity | 78 | 90 | PASS | Mixed-case + upper bound covered; normalization rule explicit |
| Edge Cases | 70 | 88 | PASS | Baseline, missing dir, non-UTF8, depth limit all covered |
| Scope Creep | 82 | 88 | PASS | v2 loophole explicitly deferred; `--audit-report` permanent |
| Dependencies | 90 | 92 | PASS | Clean graph preserved; US-006 correctly depends on US-005 |
| Testability | 85 | 90 | PASS | AC-004.1 row semantics exact; ordering fully deterministic |
| Decision Completeness | 68 | 82 | PASS | 15 decisions with substantive rationale; semver-anchored deprecation |
| Rollback & Failure | 55 | 80 | PASS | Atomic revert + env-var kill switch + refactor regression guard |
| **Average** | **74** | **87** | **PASS** | — |
...
|-----------|-----------------|
| `docs/specs/2026-04-18-claude-md-temp-marker-lint/spec.md` | Full spec (this file) |
| `docs/specs/2026-04-18-claude-md-temp-marker-lint/spec-draft.md` | Pre-PASS draft (preserved for diff) |
| `docs/specs/2026-04-18-claude-md-temp-marker-lint/campaign.md` | 11 decisions (9 grill-me + 2 adversary rounds) |
