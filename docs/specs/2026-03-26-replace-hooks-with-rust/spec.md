...
|---|----------|--------|--------|
| 1 | Audit first or port all? | Audit first — remove dead code, only port living scripts | Recommended |
| 2 | Architecture: extend ecc-hook or new crate? | Separate crate — must be buildable independently of main ecc | User |
| 3 | Crate dependencies? | Depend on ecc-domain for WorkflowState types | Recommended |
| 4 | One binary or many? | Single binary with subcommand dispatch | Recommended |
| 5 | No state.json behavior? | Exit 0 with warning | User |
| 6 | Test coverage? | Transitions 100%, passive checks 80% | Recommended |
| 7 | Performance constraints? | No explicit latency target | User |
| 8 | Security implications? | No external APIs, local files only | Recommended |
| 9 | Breaking changes? | New contract: dual mode (CLI args + stdin JSON) | User |
| 10 | New contract details? | Both: JSON output + CLI args | User |
| 11 | Domain terms? | Add WorkflowState and Phase to glossary | Recommended |
| 12 | ADR needed? | Yes — for architecture approach (separate crate) | Recommended |

### User Stories

| ID | Title | AC Count | Dependencies |
|----|-------|----------|--------------|
| US-001 | Audit and classify hook scripts | 3 | none |
| US-002 | Model WorkflowState domain aggregate | 6 | none |
| US-003 | Create ecc-workflow crate with single binary | 8 | US-002 |
| US-004 | Port active workflow scripts | 6 | US-001, US-003 |
| US-005 | Update commands and skills | 4 | US-004 |
| US-006 | Delete shell scripts and update docs | 4 | US-005 |

### Acceptance Criteria

| AC ID | Description | Source US |
|-------|-------------|----------|
| AC-001.1 | Classify all 13 scripts as active/dead | US-001 |
| AC-001.2 | Delete dead scripts | US-001 |
| AC-001.3 | Document active inventory | US-001 |
| AC-002.1 | WorkflowState fields | US-002 |
| AC-002.2 | Legal transitions + aliases | US-002 |
| AC-002.3 | Illegal transition → domain error | US-002 |
| AC-002.4 | JSON round-trip | US-002 |
| AC-002.5 | Idempotent re-entry | US-002 |
| AC-002.6 | Corrupted JSON error | US-002 |
| AC-003.1-3.8 | Binary build, subcommands, dual interface | US-003 |
| AC-004.1-4.6 | Port each active script | US-004 |
| AC-005.1-5.4 | Update all references | US-005 |
| AC-006.1-6.4 | Delete scripts, update docs | US-006 |

### Adversary Findings

| Dimension | Verdict | Key Rationale |
|-----------|---------|---------------|
| Completeness | PASS | All 13 scripts listed, env vars constrained |
| Correctness | PASS | Transitions match shell code, dual invocation model |
| Testability | PASS | ACs specific, equivalence defined |
| Feasibility | PASS | 8th crate in workspace, memory-writer is complex but feasible |
| Ambiguity | PASS | Round 1 gaps resolved |
| Scope | PASS | Non-requirements well bounded |
| Architecture | PASS | Hexagonal alignment, domain purity preserved |
