...
|---|----------|--------|--------|
| 1 | Root cause: missing status transition mechanism + dual source of truth? | Agreed — structural root cause, 34 mismatches are symptoms | Recommended |
| 2 | Minimal patch vs proper structural fix? | Proper fix: add `update-status` CLI + fix data | Recommended |
| 3 | Which test gaps to address? | Full lifecycle tests: unit + integration + round-trip | Recommended |
| 4 | Fix both agent + templates, or defer? | Fix both now — prevents drift recurrence | Recommended |
| 5 | Address ERR-004 swallowed lock error in same fix? | Yes — already in module, 1-line change | Recommended |
| 6 | Add dry-run/warning safeguard to reindex? | Yes — dry-run + warning when >5 status changes | Recommended |
| 7 | Normalize YAML quoting during migration? | Yes — normalize to unquoted for grep reliability | Recommended |
| Root cause | Dual source of truth: reindex reads files, manual edits write index. No automated `open→implemented` transition exists. | — | Investigation |

### User Stories

| ID | Title | AC Count | Dependencies |
|----|-------|----------|--------------|
| US-001 | Update backlog entry status via CLI | 7 | none |
| US-002 | Sync divergent entries via migration | 8 | US-001 |
| US-003 | Reindex safety warning | 2 | none |
| US-004 | Fix ERR-004 (swallowed lock error) | 1 | none |

### Acceptance Criteria

| AC ID | Description | Source US |
|-------|-------------|----------|
| AC-001.1 | In-place frontmatter status update, body preserved | US-001 |
| AC-001.2 | Auto-reindex after update | US-001 |
| AC-001.3 | Invalid ID error (exit 1) | US-001 |
| AC-001.4 | Invalid status error, Unknown rejected | US-001 |
| AC-001.5 | No-op guard (same status = no write) | US-001 |
| AC-001.6 | Missing status field = MalformedYaml error | US-001 |
| AC-001.7 | Duplicate status key: first occurrence updated | US-001 |
| AC-002.1 | Dynamic divergence computation (no hardcoded IDs) | US-002 |
| AC-002.2 | Open→implemented file sync | US-002 |
| AC-002.3 | Open→archived for BL-039/BL-042 | US-002 |
| AC-002.4 | YAML status quoting normalized to unquoted | US-002 |
| AC-002.5 | Idempotent proof via reindex dry-run | US-002 |
| AC-002.6 | Internal migrate_statuses() function | US-002 |
| AC-002.7 | Best-effort partial failure handling | US-002 |
| AC-002.8 | Atomic git commit for migration | US-002 |
| AC-003.1 | Block reindex >5 changes (exit 2, stderr warning) | US-003 |
| AC-003.2 | --force bypasses safety warning | US-003 |
| AC-004.1 | Log lock removal failures via tracing::warn! | US-004 |

### Adversary Findings

| Dimension | Score | Verdict | Key Rationale |
|-----------|-------|---------|---------------|
| Ambiguity | 82 | PASS | In-place defined, numbers dynamic, ERR-004 has AC |
| Edge Cases | 78 | PASS | No-op, duplicate keys, partial failure covered |
| Scope Creep Risk | 92 | PASS | Non-requirements explicit, ERR-004 justified |
| Dependency Gaps | 90 | PASS | Migration mechanism defined, ordering resolved |
| Testability | 88 | PASS | Character-for-character, exit code 2, dry-run proof |
| Decision Completeness | 80 | PASS | Transition freedom, type safety, quoting scope documented |
| Rollback & Failure | 78 | PASS | Atomic commit, best-effort, atomic write constraint |
...
| docs/specs/2026-04-17-backlog-status-conformance-fix/spec-draft.md | Draft (pre-adversary) |
