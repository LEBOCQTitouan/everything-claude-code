...
|---|----------|--------|--------|
| 1 | Scope boundaries | Include BL-072 (tasks.md generation from design PCs) in scope | User |
| 2 | Edge cases (pipe separator) | Use `→` separator instead of `\|` for status trail | User |
| 3 | Test strategy | 100% branch coverage for all ecc-domain task logic, 80% for CLI | User |
| 4 | Performance constraints | <50ms target, synchronous Rust, no async needed | Recommended |
| 5 | Security implications | Add path traversal protection for tasks-path argument | User |
| 6 | Breaking changes | Backward compat: parser reads both old `\|` and new `→` formats | Recommended |
| 7 | Domain concepts | TaskStatus, TaskEntry, TaskReport, Status Trail added to glossary | Recommended |
| 8 | ADR decisions | Two ADRs: (1) task state source of truth, (2) artifact scaffolding | User |

### User Stories

| ID | Title | AC Count | Dependencies |
|----|-------|----------|--------------|
| US-001 | tasks.md Parser (Domain Model) | 7 | none |
| US-002 | Status Transition Validation | 8 | US-001 |
| US-003 | `ecc-workflow tasks sync` Subcommand | 5 | US-001 |
| US-004 | `ecc-workflow tasks update` Subcommand | 8 | US-001, US-002 |
| US-005 | `ecc-workflow tasks init` Subcommand (BL-072) | 9 | US-001 |
| US-006 | `/implement` Command Integration | 6 | US-003, US-004, US-005 |

### Acceptance Criteria

| AC ID | Description | Source US |
|-------|-------------|----------|
| AC-001.1 | Well-formed tasks.md returns structured TaskReport | US-001 |
| AC-001.2 | Mixed `[x]`/`[ ]` correctly identified | US-001 |
| AC-001.3 | Multi-segment status trails extracted in order | US-001 |
| AC-001.4 | Post-TDD entries parsed as distinct types | US-001 |
| AC-001.5 | Malformed input returns error, never panics | US-001 |
| AC-001.6 | Old-format `\|` separator read correctly | US-001 |
| AC-001.7 | Empty tasks.md returns zero entries | US-001 |
| AC-002.1 | pending → red allowed | US-002 |
| AC-002.2 | red → green allowed | US-002 |
| AC-002.3 | green → done allowed | US-002 |
| AC-002.4 | pending → green rejected | US-002 |
| AC-002.5 | done → any rejected (terminal) | US-002 |
| AC-002.6 | red/green → failed allowed | US-002 |
| AC-002.7 | failed → red allowed (retry) | US-002 |
| AC-002.8 | Post-TDD pending → done allowed | US-002 |
| AC-003.1 | Sync outputs JSON with arrays and counters | US-003 |
| AC-003.2 | Pending contains only undone items | US-003 |
| AC-003.3 | Nonexistent path returns block error | US-003 |
| AC-003.4 | Malformed tasks.md returns warn | US-003 |
| AC-003.5 | Path traversal rejected | US-003 |
| AC-004.1 | Update appends `→ red@<ISO>` to trail | US-004 |
| AC-004.2 | Done transition changes `[ ]` to `[x]` | US-004 |
| AC-004.3 | Invalid transition rejected (pending → done for TDD) | US-004 |
| AC-004.4 | Nonexistent PC returns block error | US-004 |
| AC-004.5 | Concurrent writes handled atomically | US-004 |
| AC-004.6 | Post-TDD entry updated by string identifier | US-004 |
| AC-004.7 | Path traversal rejected | US-004 |
| AC-004.8 | Same-state transition rejected as no-op | US-004 |
| AC-005.1 | Design PCs generate tasks.md in dependency order | US-005 |
| AC-005.2 | Post-TDD section included | US-005 |
| AC-005.3 | Dependencies respected in ordering | US-005 |
| AC-005.4 | Initial `pending@<ISO>` timestamp set | US-005 |
| AC-005.5 | Existing output blocked without --force | US-005 |
| AC-005.6 | New `→` separator used | US-005 |
| AC-005.7 | No PC table returns block error | US-005 |
| AC-005.8 | Duplicate PC IDs return block error | US-005 |
| AC-005.9 | --force flag overwrites existing | US-005 |
| AC-006.1 | Phase 2 uses `tasks init` | US-006 |
| AC-006.2 | Phase 2 uses `tasks sync` for TodoWrite | US-006 |
| AC-006.3 | Phase 3 syncs before each wave | US-006 |
| AC-006.4 | Parent calls `tasks update` after tdd-executor | US-006 |
| AC-006.5 | Re-entry uses sync to rebuild state | US-006 |
| AC-006.6 | TaskCreate derived from sync output | US-006 |

### Adversary Findings

| Dimension | Score | Verdict | Key Rationale |
|-----------|-------|---------|---------------|
| Ambiguity | 72 | PASS | Post-TDD identifier scheme and JSON schema could be more explicit |
| Edge Cases | 82 | PASS | All 5 required ACs added in round 2 |
| Scope | 82 | PASS | BL-072 bundled but well-bounded by Non-Requirements |
| Dependencies | 78 | PASS | ecc-flock transitive dependency noted |
| Testability | 85 | PASS | Domain pure functions excellent for testing |
| Decisions | 88 | PASS | All 10 decisions well-justified with ADR refs |
| Rollback | 90 | PASS | All changes additive, safely revertible |

### Artifacts Persisted

| File Path | Section Written |
|-----------|-----------------|
| docs/specs/2026-03-29-deterministic-task-sync/spec.md | Full spec |
