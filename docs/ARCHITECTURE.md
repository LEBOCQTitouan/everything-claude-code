...
| **Source files** | 48 `.ts` files under `src/` |
| **Codebase size** | Small (48 source files) |
| **Entry points** | `bin/ecc.js` (CLI), `dist/postinstall.js`, `dist/preuninstall.js`, `dist/install-orchestrator.js`, `dist/claw.js` |
| **Test count** | 1272 (27 test files, single-process runner) |
...
User CLI (ecc)
  ...
  +-- Content (copied to ~/.claude/)
       +-- agents/    (25 markdown with YAML frontmatter)
       +-- commands/  (47 markdown slash commands)
       +-- skills/    (70 domain knowledge directories)
...
| **Agent Orchestration** | 25 agent markdown files with delegation patterns (planner, tdd-guide, code-reviewer, architect, 6-agent doc system with diagram-generator, etc.) | Medium |
