...
function validateHookEntry(hook: HookEntry, label: string): boolean {
  let hasErrors = false;
  ...
  if (
    ...
    (Array.isArray(hook.command) && (hook.command.length === 0 || !hook.command.every(s => typeof s === 'string' && s.length > 0)))
  ) {
    ...
    if (nodeEMatch) {
      try {
        ...
        console.error(`ERROR: ${label} has invalid inline JS: ${(syntaxErr as Error).message}`);
        hasErrors = true;
      }
    }
  }
...
function validateHooks(): void {
  ...
  let data: { hooks?: Record<string, MatcherEntry[]> | MatcherEntry[] };
  try {
    data = JSON.parse(fs.readFileSync(HOOKS_FILE, 'utf-8'));
  } catch (e) {
    console.error(`ERROR: Invalid JSON in hooks.json: ${(e as Error).message}`);
    process.exit(1);
  }
  ...
  if (typeof hooks === 'object' && !Array.isArray(hooks)) {
    for (const [eventType, matchers] of Object.entries(hooks as Record<string, MatcherEntry[]>)) {
    ...
    for (let i = 0; i < hooks.length; i++) {
      const hook = hooks[i] as MatcherEntry;
      if (!hook.matcher) {
