...
3. Tokenize the bug description into words. Perform exact match (case-insensitive) against bounded-contexts module names.
...
7. Spawn each matched domain agent as a read-only Task subagent with `allowedTools: [Read, Grep, Glob]` and prompt: "Summarize your domain knowledge relevant to: <bug description>".
