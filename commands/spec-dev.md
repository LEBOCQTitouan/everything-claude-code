...
Invoke the CLI via the Bash tool (do NOT use `!`-prefix shell eval) to initialize the workflow state. Pass the feature description via an environment variable and pipe it through stdin to avoid shell-argv interpolation of metacharacters. For example: `env FEATURE='<user's feature description>' sh -c 'printf %s "$FEATURE" | ecc-workflow init dev --feature-stdin'`. This pattern is safe for all characters including backticks, quotes, dollar signs, and backslashes.

### Worktree Isolation

Generate a worktree name and isolate this session:
1. Invoke the CLI via the Bash tool to generate the worktree name. Pass the feature description via an environment variable and pipe it through stdin, e.g.: `env FEATURE='<user's feature description>' sh -c 'printf %s "$FEATURE" | ecc-workflow worktree-name dev --feature-stdin'` — capture the output name.
...
2. Read `docs/domain/bounded-contexts.md` to get the list of module names (both table rows and freestanding `###` sections).
3. Tokenize the feature description into words. Perform exact match (case-insensitive) of each word against the bounded-contexts module names.
...
7. Spawn each matched domain agent as a read-only Task subagent with `allowedTools: [Read, Grep, Glob]` and prompt: "Summarize your domain knowledge relevant to: <feature description>".
8. Collect the output from each domain agent and store it verbatim in a `## Domain Context` section prepended to the plan file, available as pre-context for all subsequent phases.
