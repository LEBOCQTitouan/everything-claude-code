...
Invoke the CLI via the Bash tool (do NOT use `!`-prefix shell eval) to initialize the workflow state for this refactor. Pass the refactoring description via an environment variable and pipe it through stdin to avoid shell-argv interpolation of metacharacters. For example: `env FEATURE='<user's refactoring description>' sh -c 'printf %s "$FEATURE" | ecc-workflow init refactor --feature-stdin'`. This pattern is safe for all characters including backticks, quotes, dollar signs, and backslashes.

### Worktree Isolation

Generate a worktree name and isolate this session:
1. Invoke the CLI via the Bash tool to generate the worktree name. Pass the refactoring description via an environment variable and pipe it through stdin, e.g.: `env FEATURE='<user's refactoring description>' sh -c 'printf %s "$FEATURE" | ecc-workflow worktree-name refactor --feature-stdin'` — capture the output name.
