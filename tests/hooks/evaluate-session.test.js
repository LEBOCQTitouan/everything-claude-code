...
async function runTests() {
  describe('Testing evaluate-session.js');


  // Threshold boundary tests (default minSessionLength = 10)
  describe('Threshold boundary (default min=10)');


  await test('skips session with 9 user messages (below threshold)', () => {
      ...
      assert.ok(result.stderr.includes('too short') || result.stderr.includes('9 messages'), 'Should indicate session too short');
      cleanupTestDir(testDir);
    });

  await test('evaluates session with exactly 10 user messages (at threshold)', () => {
      const testDir = createTestDir();
      ...
      assert.ok(result.stderr.includes('10 messages') || result.stderr.includes('evaluate'), 'Should indicate evaluation');
      cleanupTestDir(testDir);
    });

  await test('evaluates session with 11 user messages (above threshold)', () => {
      const testDir = createTestDir();
  ...
  await test('skips empty transcript file (0 user messages)', () => {
      const testDir = createTestDir();
      ...
      assert.ok(result.stderr.includes('too short') || result.stderr.includes('0 messages'), 'Empty transcript should be too short');
      cleanupTestDir(testDir);
    });

  await test('counts only user messages (ignores assistant messages)', () => {
      const testDir = createTestDir();
      ...
      const result = runEvaluate({ transcript_path: filePath });
      assert.strictEqual(result.code, 0);
      assert.ok(result.stderr.includes('too short') || result.stderr.includes('5 messages'), 'Should count only user messages');
      cleanupTestDir(testDir);
    });
  ...
  await test('uses custom min_session_length from config file', () => {
      const testDir = createTestDir();
      ...
      assert.ok(result.stderr.includes('too short') || result.stderr.includes('4 messages'), 'With default config, 4 messages should be too short');
      cleanupTestDir(testDir);
    });

  await test('handles transcript with only assistant messages (0 user match)', () => {
      const testDir = createTestDir();
      ...
      assert.ok(result.stderr.includes('too short') || result.stderr.includes('0 messages'), 'Should report too short with 0 user messages');
      cleanupTestDir(testDir);
    });

  await test('handles transcript with malformed JSON lines (still counts valid ones)', () => {
      const testDir = createTestDir();
      const filePath = path.join(testDir, 'mixed.jsonl');
      ...
      assert.ok(result.stderr.includes('evaluate') && result.stderr.includes('12 messages'), 'Should evaluate session with 12 valid user messages');
      cleanupTestDir(testDir);
    });

  await test('handles empty stdin (no input) gracefully', () => {
      const result = spawnSync('node', [evaluateScript], {
        encoding: 'utf8',
        input: '',
        timeout: 10000
  ...
  describe('Round 53: CLAUDE_TRANSCRIPT_PATH fallback');


  await test('falls back to CLAUDE_TRANSCRIPT_PATH env var when stdin is invalid JSON', () => {
      const testDir = createTestDir();
      ...
      assert.ok(result.stderr.includes('15 messages'), 'Should evaluate using env var fallback path');
  ...
  describe('Round 65: regex whitespace tolerance around colon');


  await test('counts user messages when JSON has spaces around colon ("type" : "user")', () => {
      const testDir = createTestDir();
      ...
      assert.ok(!result.stderr.includes('too short'), 'Should NOT say too short for 12 spaced-colon user messages');
      assert.ok(result.stderr.includes('12 messages') || result.stderr.includes('evaluate'), `Should evaluate session with spaced-colon JSON. Got stderr: ${result.stderr}`);
      cleanupTestDir(testDir);
    });

  // ── Round 85: config file parse error (corrupt JSON) ──
  describe('Round 85: config parse error catch block');


  await test('falls back to defaults when config file contains invalid JSON', () => {
      ...
      try {
        ...
        assert.ok(!result.stderr.includes('too short'), `Should NOT say too short — corrupt config falls back to default min=10. Got: ${result.stderr}`);
        assert.ok(result.stderr.includes('12 messages') || result.stderr.includes('evaluate'), `Should evaluate with 12 messages using default threshold. Got: ${result.stderr}`);
        // The catch block logs "Failed to parse config" — verify that log message
        assert.ok(result.stderr.includes('Failed to parse config'), `Should log config parse error. Got: ${result.stderr}`);
  ...
  describe('Round 86: config learned_skills_path override');


  await test('uses learned_skills_path from config with ~ expansion', () => {
      ...
      try {
        ...
        assert.ok(!result.stderr.includes('~/custom-learned-skills-dir'), 'Should NOT contain literal ~ in output (should be expanded)');
        assert.ok(result.stderr.includes('custom-learned-skills-dir'), `Should reference the custom learned skills dir. Got: ${result.stderr}`);
        // The ~ should have been replaced with os.homedir()
        assert.ok(result.stderr.includes(os.homedir()), `Should contain expanded home directory. Got: ${result.stderr}`);
