...
async function runTests() {
  ...
  describe('session-start.js (edge cases)');

  await test('exits 0 even with isolated empty HOME', async () => {
  ...
  await test('injects real session content', async () => {
  ...
  await test('reports learned skills count', async () => {
  ...
  await test('passes through stdin data to stdout', async () => {
  ...
  await test('exits 0 with empty stdin', async () => {
    const result = await runScript(path.join(scriptsDir, 'check-console-log.js'), '');
    assert.strictEqual(result.code, 0);
  });

  await test('handles invalid JSON stdin gracefully', async () => {
  ...
  await test('creates or updates session file', async () => {
    // Run the script
  ...
  await test('includes session ID in filename', async () => {
  ...
  await test('outputs PreCompact message', async () => {
    const result = await runScript(path.join(scriptsDir, 'pre-compact.js'));
    assert.ok(result.stderr.includes('[PreCompact]'), 'Should output PreCompact message');
  });

  await test('creates compaction log', async () => {
  ...
  await test('annotates active session file with compaction marker', async () => {
  ...
  await test('compaction log contains timestamp', async () => {
  ...
  await test('increments counter on each call', async () => {
  ...
  await test('suggests compact at threshold', async () => {
  ...
  await test('does not suggest below threshold', async () => {
  ...
  await test('suggests at regular intervals after threshold', async () => {
  ...
  await test('handles corrupted counter file', async () => {
  ...
  await test('uses default session ID when no env var', async () => {
    const result = await runScript(path.join(scriptsDir, 'suggest-compact.js'), '', {
  ...
  await test('validates threshold bounds', async () => {
  ...
  await test('runs without error when no transcript', async () => {
    const result = await runScript(path.join(scriptsDir, 'evaluate-session.js'));
    assert.strictEqual(result.code, 0, `Exit code should be 0, got ${result.code}`);
  });

  await test('skips short sessions', async () => {
  ...
  await test('processes sessions with enough messages', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('counts user messages with whitespace in JSON (regression)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('handles transcript with null content array elements (regression)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  describe('post-edit-console-warn.js');

  await test('warns about console.log in JS files', async () => {
    const testDir = createTestDir();
  ...
  await test('does not warn for non-JS files', async () => {
    const testDir = createTestDir();
  ...
  await test('does not warn for clean JS files', async () => {
    const testDir = createTestDir();
  ...
  await test('handles missing file gracefully', async () => {
  ...
  await test('limits console.log output to 5 matches', async () => {
    const testDir = createTestDir();
  ...
  await test('ignores console.warn and console.error (only flags console.log)', async () => {
    const testDir = createTestDir();
  ...
  await test('passes through original data on stdout', async () => {
  ...
  await test('runs without error on empty stdin', async () => {
  ...
  await test('skips non-JS/TS files', async () => {
    const stdinJson = JSON.stringify({ tool_input: { file_path: '/test.py' } });
  ...
  await test('passes through data for invalid JSON', async () => {
  ...
  await test('handles null tool_input gracefully', async () => {
  ...
  await test('handles missing file_path in tool_input', async () => {
  ...
  await test('exits 0 and passes data when prettier is unavailable', async () => {
  ...
  await test('skips non-TypeScript files', async () => {
  ...
  await test('handles nonexistent TS file gracefully', async () => {
    const stdinJson = JSON.stringify({ tool_input: { file_path: '/nonexistent/file.ts' } });
    const result = await runScript(path.join(scriptsDir, 'post-edit-typecheck.js'), stdinJson);
    assert.strictEqual(result.code, 0, 'Should exit 0 for missing file');
  });

  await test('handles TS file with no tsconfig gracefully', async () => {
    const testDir = createTestDir();
  ...
  await test('stops tsconfig walk at max depth (20)', async () => {
  ...
  await test('passes through stdin data on stdout (post-edit-typecheck)', async () => {
    const testDir = createTestDir();
    const testFile = path.join(testDir, 'test.ts');
    fs.writeFileSync(testFile, 'const x: number = 1;');
  ...
  describe('session-end.js (extractSessionSummary)');

  await test('extracts user messages from transcript', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('handles transcript with array content fields', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('extracts tool names and file paths from transcript', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('handles transcript with malformed JSON lines', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('handles empty transcript (no user messages)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('truncates long user messages to 200 chars', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('uses CLAUDE_TRANSCRIPT_PATH env var as fallback', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('escapes backticks in user messages in session file', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('session file contains tools used and files modified', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('omits Tools Used and Files Modified sections when empty', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('slices user messages to last 10', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('slices tools to first 20', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('slices files modified to first 30', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('parses Claude Code JSONL format (entry.message.content)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('extracts tool_use from assistant message content blocks', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('hooks.json is valid JSON', () => {
    const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
    const content = fs.readFileSync(hooksPath, 'utf8');
    JSON.parse(content); // Will throw if invalid
  });

  await test('hooks.json has required event types', () => {
    const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
  ...
  await test('hooks.json exists in hooks/', () => {
    const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
  ...
  await test('skips when no transcript_path in stdin', async () => {
    const result = await runScript(path.join(scriptsDir, 'evaluate-session.js'), '{}');
    assert.strictEqual(result.code, 0, 'Should exit 0 (non-blocking)');
  });

  await test('skips when transcript file does not exist', async () => {
  ...
  await test('skips short sessions (< 10 user messages)', async () => {
    const testDir = createTestDir();
  ...
  await test('evaluates long sessions (>= 10 user messages)', async () => {
    const testDir = createTestDir();
  ...
  await test('handles malformed stdin JSON (falls back to env var)', async () => {
  ...
  await test('increments tool counter on each invocation', async () => {
    const sessionId = `test-counter-${Date.now()}`;
    const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
    try {
  ...
  await test('suggests compact at exact threshold', async () => {
    const sessionId = `test-threshold-${Date.now()}`;
    const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
    try {
  ...
  await test('suggests at periodic intervals after threshold', async () => {
  ...
  await test('resets counter when file contains huge overflow number', async () => {
  ...
  await test('resets counter when file contains negative number', async () => {
  ...
  await test('handles COMPACT_THRESHOLD of zero (falls back to 50)', async () => {
  ...
  await test('handles invalid COMPACT_THRESHOLD (falls back to 50)', async () => {
  ...
  describe('check-console-log.js (exact pass-through)');

  await test('stdout is exact byte match of stdin (no trailing newline)', async () => {
  ...
  await test('preserves empty string stdin without adding newline', async () => {
    const result = await runScript(path.join(scriptsDir, 'check-console-log.js'), '');
    assert.strictEqual(result.code, 0);
    assert.strictEqual(result.stdout, '', 'Empty input should produce empty output');
  });

  await test('preserves data with embedded newlines exactly', async () => {
    const stdinData = 'line1\nline2\nline3';
    const result = await runScript(path.join(scriptsDir, 'check-console-log.js'), stdinData);
    assert.strictEqual(result.code, 0);
    assert.strictEqual(result.stdout, stdinData, 'Should preserve embedded newlines without adding extra');
  });
  describe('post-edit-format.js (security & extension tests)');

  await test('source code does not pass shell option to execFileSync (security)', async () => {
  ...
  await test('matches .tsx extension for formatting', async () => {
  ...
  await test('matches .jsx extension for formatting', async () => {
  ...
  describe('post-edit-typecheck.js (security & extension tests)');
  ...
  await test('matches .tsx extension for type checking', async () => {
    const testDir = createTestDir();
  ...
  describe('Round 23: evaluate-session.js (config & nullish coalescing)');

  await test('respects min_session_length=0 from config (nullish coalescing)', async () => {
  ...
  await test('config with min_session_length=null falls back to default 10', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'short.jsonl');
  ...
  await test('config with custom learned_skills_path creates directory', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('handles invalid config JSON gracefully (uses defaults)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
    const lines = [];
    for (let i = 0; i < 5; i++) lines.push(`{"type":"user","content":"msg${i}"}`);
  ...
  describe('Round 23: session-end.js (update existing file path)');

  await test('updates Last Updated timestamp in existing session file', async () => {
    const testDir = createTestDir();
  ...
  await test('replaces blank template with summary when updating existing file', async () => {
    const testDir = createTestDir();
    const sessionsDir = path.join(testDir, '.claude', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  await test('always updates session summary content on session end', async () => {
    const testDir = createTestDir();
    const sessionsDir = path.join(testDir, '.claude', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  describe('Round 23: pre-compact.js (glob specificity)');

  await test('only annotates *-session.tmp files, not other .tmp files', async () => {
  ...
  await test('handles no active session files gracefully', async () => {
  ...
  describe('Round 23: session-end.js (extractSessionSummary edge cases)');

  await test('handles transcript with only assistant messages (no user messages)', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  describe('Round 24: suggest-compact.js (interval fix & fd fallback)');

  await test('periodic intervals are consistent with non-25-divisible threshold', async () => {
  ...
  await test('does not suggest at old-style multiples that skip threshold offset', async () => {
  ...
  await test('fd fallback: handles corrupted counter file gracefully', async () => {
    const sessionId = `test-corrupt-${Date.now()}`;
    const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
    try {
  ...
  await test('handles counter at exact 1000000 boundary', async () => {
  ...
  describe('Round 24: post-edit-format.js (edge cases)');

  await test('passes through malformed JSON unchanged', async () => {
  ...
  await test('passes through data for non-JS/TS file extensions', async () => {
  ...
  describe('Round 24: post-edit-typecheck.js (edge cases)');

  await test('skips typecheck for non-existent file and still passes through', async () => {
  ...
  await test('passes through for non-TS extensions without running tsc', async () => {
  ...
  describe('Round 24: session-start.js (edge cases)');

  await test('exits 0 with empty sessions directory (no recent sessions)', async () => {
  ...
  await test('does not inject blank template session into context', async () => {
  ...
  describe('Round 25: post-edit-console-warn.js (pass-through fix)');
  ...
  await test('passes through malformed JSON unchanged without crash', async () => {
  ...
  await test('handles missing file_path in tool_input gracefully', async () => {
    const stdinJson = JSON.stringify({ tool_input: {} });
    const result = await runScript(path.join(scriptsDir, 'post-edit-console-warn.js'), stdinJson);
    assert.strictEqual(result.code, 0);
  ...
  await test('passes through when file does not exist (readFile returns null)', async () => {
    const stdinJson = JSON.stringify({ tool_input: { file_path: '/nonexistent/deep/file.ts' } });
    const result = await runScript(path.join(scriptsDir, 'post-edit-console-warn.js'), stdinJson);
    assert.strictEqual(result.code, 0);
    assert.strictEqual(result.stdout, stdinJson, 'Should pass through exactly when file not found');
  });
  describe('Round 25: check-console-log.js (edge cases)');

  await test('source has expected exclusion patterns', async () => {
  ...
  await test('passes through data unchanged on non-git repo', async () => {
  ...
  await test('exits 0 even when no stdin is provided', async () => {
    const result = await runScript(path.join(scriptsDir, 'check-console-log.js'), '');
  ...
  describe('Round 29: post-edit-format.js (cwd and exit)');

  await test('source uses cwd based on file directory for npx', async () => {
    const formatSource = fs.readFileSync(path.join(srcHooksDir, 'post-edit-format.ts'), 'utf8');
  ...
  await test('source calls process.exit(0) after writing output', async () => {
  ...
  await test('uses process.stdout.write instead of console.log for pass-through', async () => {
    const formatSource = fs.readFileSync(path.join(scriptsDir, 'post-edit-format.js'), 'utf8');
  ...
  describe('Round 29: post-edit-typecheck.js (exit and pass-through)');
  ...
  await test('exact stdout pass-through without trailing newline (typecheck)', async () => {
  ...
  await test('exact stdout pass-through without trailing newline (format)', async () => {
    const stdinJson = JSON.stringify({ tool_input: { file_path: '/nonexistent/file.py' } });
    const result = await runScript(path.join(scriptsDir, 'post-edit-format.js'), stdinJson);
    assert.strictEqual(result.code, 0);
    assert.strictEqual(result.stdout, stdinJson, 'stdout should exactly match stdin (no trailing newline)');
  });
  describe('Round 29: post-edit-console-warn.js (extension and exit)');
  ...
  await test('does NOT match .mts or .mjs extensions', async () => {
  ...
  await test('does NOT match uppercase .TS extension', async () => {
  ...
  await test('detects console.log in commented-out code', async () => {
    const testDir = createTestDir();
  ...
  describe('Round 29: check-console-log.js (exclusion patterns and exit)');
  ...
  await test('EXCLUDED_PATTERNS correctly excludes test files', async () => {
  ...
  await test('exclusion patterns match expected file paths', async () => {
  ...
  describe('Round 29: run-all.js test runner improvements');

  await test('test runner uses single-process require-based orchestration', async () => {
    const runAllSource = fs.readFileSync(path.join(__dirname, '..', 'run-all.js'), 'utf8');
    // Single-process runner uses require() not spawnSync
    assert.ok(runAllSource.includes('require(testPath)'), 'Should use require() to import test files');
    assert.ok(!runAllSource.includes('execSync'), 'Should not use execSync');
    // Verify it imports the shared harness
    assert.ok(runAllSource.includes('./harness'), 'Should use shared harness');
  });

  // ── Round 32: post-edit-typecheck special characters & check-console-log ──
  describe('Round 32: post-edit-typecheck (special character paths)');

  await test('handles file path with spaces gracefully', async () => {
    const testDir = createTestDir();
  ...
  await test('handles file path with shell metacharacters safely', async () => {
    const testDir = createTestDir();
  ...
  await test('handles .tsx file extension', async () => {
    const testDir = createTestDir();
    const testFile = path.join(testDir, 'component.tsx');
  ...
  describe('Round 32: check-console-log (edge cases)');

  await test('passes through data when git commands fail', async () => {
  ...
  await test('handles very large stdin within limit', async () => {
  ...
  describe('Round 32: post-edit-console-warn (additional edge cases)');

  await test('handles file with only console.error (no warning)', async () => {
    const testDir = createTestDir();
  ...
  describe('Round 32: session-end.js (empty transcript)');

  await test('handles completely empty transcript file', async () => {
    const testDir = createTestDir();
  ...
  await test('handles transcript with only whitespace lines', async () => {
    const testDir = createTestDir();
  ...
  describe('Round 38: evaluate-session.js (tilde expansion & missing config)');

  await test('expands ~ in learned_skills_path to home directory', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  await test('does NOT expand ~ in middle of learned_skills_path', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
    fs.writeFileSync(transcriptPath, '{"type":"user","content":"msg"}');
  ...
  await test('uses defaults when config file does not exist', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  describe('Round 41: pre-compact.js (multiple session files)');

  await test('annotates only the newest session file when multiple exist', async () => {
  ...
  describe('Round 40: session-end.js (newline collapse)');

  await test('collapses newlines in user messages to single-line markdown items', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  describe('Round 44: session-start.js (empty session file)');

  await test('does not inject empty session file content into context', async () => {
  ...
  describe('Round 49: post-edit-typecheck.js (extension edge cases)');

  await test('.d.ts files match the TS regex and trigger typecheck path', async () => {
    const testDir = createTestDir();
  ...
  await test('.mts extension does not trigger typecheck', async () => {
  ...
  describe('Round 49: session-end.js (conditional summary sections)');

  await test('summary omits Files Modified and Tools Used when none found', async () => {
  ...
  describe('Round 50: session-start.js (alias reporting)');

  await test('reports available session aliases on startup', async () => {
  ...
  describe('Round 50: pre-compact.js (parallel execution)');

  await test('parallel compaction runs all append to log without loss', async () => {
  ...
  describe('Round 50: session-start.js (graceful degradation)');

  await test('exits 0 when sessions path is a file (not a directory)', async () => {
  ...
  describe('Round 53: post-edit-console-warn.js (max matches truncation)');

  await test('reports maximum 5 console.log matches per file', async () => {
    const testDir = createTestDir();
    const testFile = path.join(testDir, 'many-logs.js');
    const lines = Array(7)
      .fill(null)
  ...
  describe('Round 53: post-edit-format.js (non-existent file)');

  await test('passes through data for non-existent .tsx file path', async () => {
  ...
  describe('Round 55: session-start.js (maxAge 7-day boundary)');

  await test('excludes session files older than 7 days', async () => {
  ...
  describe('Round 55: session-start.js (newest session selection)');

  await test('injects newest session when multiple recent sessions exist', async () => {
  ...
  describe('Round 55: session-end.js (stdin overflow)');

  await test('handles stdin exceeding MAX_STDIN (1MB) gracefully', async () => {
    const testDir = createTestDir();
    const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  describe('Round 56: post-edit-typecheck.js (tsconfig in parent directory)');

  await test('walks up directory tree to find tsconfig.json in grandparent', async () => {
    const testDir = createTestDir();
  ...
  describe('Round 56: suggest-compact.js (counter file as directory — fallback path)');

  await test('exits 0 when counter file path is occupied by a directory', async () => {
  ...
  describe('Round 59: session-start.js (unreadable session file — readFile returns null)');

  await test('does not inject content when session file is unreadable', async () => {
  ...
  describe('Round 59: check-console-log.js (stdin exceeding 1MB — truncation)');

  await test('truncates stdin at 1MB limit and still passes through data', async () => {
  ...
  describe('Round 59: pre-compact.js (read-only session file — appendFile error)');

  await test('exits 0 when session file is read-only (appendFile fails)', async () => {
    if (process.platform === 'win32' || (process.getuid && process.getuid() === 0)) {
      console.log('    (skipped — not supported on this platform)');
      return;
    }
  ...
  describe('Round 60: session-end.js (replaceInFile returns false — timestamp update warning)');

  await test('logs warning when existing session file lacks Last Updated field', async () => {
  ...
  describe('Round 60: post-edit-console-warn.js (stdin exceeding 1MB — truncation)');
  ...
  describe('Round 60: post-edit-format.js (valid JSON without tool_input key)');

  await test('skips formatting when JSON has no tool_input field', async () => {
  ...
  describe('Round 64: post-edit-typecheck.js (valid JSON without tool_input)');

  await test('skips typecheck when JSON has no tool_input field', async () => {
  ...
  describe('Round 66: session-end.js (entry.role user fallback)');

  await test('extracts user messages from role-only format (no type field)', async () => {
  ...
  describe('Round 66: session-end.js (nonexistent transcript path)');

  await test('logs "Transcript not found" for nonexistent transcript_path', async () => {
  ...
  describe('Round 70: session-end.js (entry.name/entry.input fallback)');

  await test('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
  ...
  describe('Round 71: session-start.js (default source — selection prompt)');

  await test('shows selection prompt when no package manager preference found (default source)', async () => {
  ...
  describe('Round 74: session-start.js (main catch — unrecoverable error)');

  await test('session-start exits 0 with error message when HOME is non-directory', async () => {
  ...
  describe('Round 75: pre-compact.js (main catch — unrecoverable error)');

  await test('pre-compact exits 0 with error message when HOME is non-directory', async () => {
    if (process.platform === 'win32') {
      console.log('    (skipped — /dev/null not available on Windows)');
      return;
    }
  ...
  describe('Round 75: session-end.js (main catch — unrecoverable error)');

  await test('session-end exits 0 with error message when HOME is non-directory', async () => {
    if (process.platform === 'win32') {
      console.log('    (skipped — /dev/null not available on Windows)');
      return;
    }
  ...
  describe('Round 76: evaluate-session.js (main catch — unrecoverable error)');

  await test('evaluate-session exits 0 with error message when HOME is non-directory', async () => {
    if (process.platform === 'win32') {
      console.log('    (skipped — /dev/null not available on Windows)');
      return;
    }
  ...
  describe('Round 76: suggest-compact.js (main catch — double-failure)');

  await test('suggest-compact exits 0 with error when TMPDIR is non-directory', async () => {
    if (process.platform === 'win32') {
      console.log('    (skipped — /dev/null not available on Windows)');
      return;
    }
  ...
  describe('Round 80: session-end.js (entry.message.role user — third OR condition)');

  await test('extracts user messages from entries where only message.role is user (not type or role)', async () => {
  ...
  describe('Round 81: suggest-compact.js (COMPACT_THRESHOLD > 10000)');

  await test('COMPACT_THRESHOLD exceeding 10000 falls back to default 50', async () => {
  ...
  describe('Round 81: session-end.js (user entry with non-string non-array content)');

  await test('skips user messages with numeric content (non-string non-array branch)', async () => {
  ...
  describe('Round 82: session-end.js (entry.tool_name without type=tool_use)');

  await test('collects tool name from entry with tool_name but non-tool_use type', async () => {
  ...
  describe('Round 82: session-end.js (template marker present but regex no-match)');

  await test('preserves file when marker present but regex does not match corrupted template', async () => {
...
`;
  ...
  describe('Round 87: post-edit-format.js (stdin exceeding 1MB — truncation)');

  await test('truncates stdin at 1MB limit and still passes through data (post-edit-format)', async () => {
  ...
  describe('Round 87: post-edit-typecheck.js (stdin exceeding 1MB — truncation)');

  await test('truncates stdin at 1MB limit and still passes through data (post-edit-typecheck)', async () => {
  ...
  describe('Round 89: post-edit-typecheck.js (TypeScript error detection path)');

  await test('filters TypeScript errors to edited file when tsc reports errors', async () => {
  ...
  describe('Round 89: session-end.js (entry.name + entry.input fallback in extractSessionSummary)');

  await test('extracts tool name from entry.name and file path from entry.input (fallback format)', async () => {
  ...
  describe('Round 90: readStdinJson (timeout fires when stdin stays open)');

  await test('readStdinJson resolves with {} when stdin never closes (timeout fires, no data)', async () => {
  ...
  await test('readStdinJson resolves with {} when timeout fires with invalid partial JSON', async () => {
  ...
  describe('Round 94: session-end.js (tools used without files modified)');

  await test('session file includes Tools Used but omits Files Modified when only Read/Grep used', async () => {
