    ...
    if (
      await test('replaces CLAUDE_PLUGIN_ROOT placeholder', () => {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            hooks: {
              Stop: [{ matcher: '*', hooks: [{ type: 'command', command: 'node "${CLAUDE_PLUGIN_ROOT}/dist/test.js"' }], description: 'Test' }]
            }
          })
        );

        mergeHooks(path.join(hooksDir, 'hooks.json'), path.join(settingsDir, 'settings.json'), '/my/plugin');
