...
// Import the modules
const pm = require('../../src/lib/package-manager');
...
async function runTests() {
  describe('Testing package-manager.js');

  // PACKAGE_MANAGERS constant tests
  describe('PACKAGE_MANAGERS Constant');

  await test('PACKAGE_MANAGERS has all expected managers', () => {
  ...
  await test('Each manager has required properties', () => {
  ...
  await test('detects npm from package-lock.json', () => {
  ...
  await test('detects pnpm from pnpm-lock.yaml', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('detects yarn from yarn.lock', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('detects bun from bun.lockb', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('returns null when no lock file exists', () => {
    const testDir = createTestDir();
    try {
      const result = pm.detectFromLockFile(testDir);
  ...
  await test('respects detection priority (pnpm > npm)', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('detects package manager from packageManager field', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('handles packageManager without version', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('returns null when no packageManager field', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('returns null when no package.json exists', () => {
    const testDir = createTestDir();
    try {
      const result = pm.detectFromPackageJson(testDir);
      assert.strictEqual(result, null);
  ...
  await test('returns array of available managers', () => {
  ...
  await test('returns object with name, config, and source', () => {
  ...
  await test('respects environment variable', () => {
  ...
  await test('detects from lock file in project', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    delete process.env.CLAUDE_PACKAGE_MANAGER;
    const testDir = createTestDir();
    try {
  ...
  await test('returns correct install command', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
  ...
  await test('returns correct test command', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
  ...
  await test('returns correct exec command for npm', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('returns correct exec command for pnpm', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  await test('generates pattern for dev command', () => {
  ...
  await test('pattern matches actual commands', () => {
  ...
  await test('returns informative prompt', () => {
  ...
  describe('setProjectPackageManager');

  await test('sets project package manager', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('rejects unknown package manager', () => {
    assert.throws(() => {
  ...
  describe('setPreferredPackageManager');
  ...
  await test('handles invalid JSON in package.json', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('returns null for unknown package manager in packageManager field', () => {
    const testDir = createTestDir();
    try {
  ...
  describe('getExecCommand (edge cases)');

  await test('returns exec command without args', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('getRunCommand (additional)');

  await test('returns correct build command', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('returns correct dev command', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('returns correct custom script command', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('generates pattern for custom action', () => {
  ...
  describe('getPackageManager (robustness)');

  await test('falls through on corrupted project config JSON', () => {
  ...
  await test('falls through on project config with unknown PM', () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pm-robust-'));
    const claudeDir = path.join(testDir, '.claude');
    fs.mkdirSync(claudeDir, { recursive: true });
  ...
  describe('getRunCommand (validation)');

  await test('rejects empty script name', () => {
    assert.throws(() => pm.getRunCommand(''), /non-empty string/);
  });

  await test('rejects null script name', () => {
    assert.throws(() => pm.getRunCommand(null), /non-empty string/);
  });

  await test('rejects script name with shell metacharacters', () => {
    assert.throws(() => pm.getRunCommand('test; rm -rf /'), /unsafe characters/);
  });

  await test('rejects script name with backticks', () => {
    assert.throws(() => pm.getRunCommand('test`whoami`'), /unsafe characters/);
  });

  await test('accepts scoped package names', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('getExecCommand (validation)');

  await test('rejects empty binary name', () => {
    assert.throws(() => pm.getExecCommand(''), /non-empty string/);
  });

  await test('rejects null binary name', () => {
    assert.throws(() => pm.getExecCommand(null), /non-empty string/);
  });

  await test('rejects binary name with shell metacharacters', () => {
    assert.throws(() => pm.getExecCommand('prettier; cat /etc/passwd'), /unsafe characters/);
  });

  await test('accepts dotted binary names like tsc', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('getPackageManager (source detection)');

  await test('detects from valid project-config (.claude/package-manager.json)', () => {
  ...
  await test('project-config takes priority over package.json', () => {
  ...
  await test('package.json takes priority over lock file', () => {
  ...
  await test('defaults to npm when no config found', () => {
  ...
  describe('setPreferredPackageManager (success)');

  await test('successfully saves preferred package manager', () => {
  ...
  describe('getCommandPattern (completeness)');

  await test('generates pattern for test command', () => {
    const pattern = pm.getCommandPattern('test');
  ...
  await test('generates pattern for build command', () => {
  ...
  describe('getRunCommand (PM-specific formats)');

  await test('pnpm custom script: pnpm (no run keyword)', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  await test('yarn custom script: yarn <script>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  await test('bun custom script: bun run <script>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
  ...
  await test('npm custom script: npm run <script>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
      const cmd = pm.getRunCommand('lint');
  ...
  await test('pnpm install returns pnpm install', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  await test('yarn install returns yarn (no install keyword)', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  await test('bun test returns bun test', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'bun';
  ...
  describe('getExecCommand (PM-specific formats)');

  await test('pnpm exec: pnpm dlx <binary>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  await test('yarn exec: yarn dlx <binary>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  await test('bun exec: bunx <binary>', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'bun';
  ...
  await test('ignores unknown env var package manager', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
  ...
  describe('getRunCommand (non-string input)');

  await test('rejects undefined script name', () => {
    assert.throws(() => pm.getRunCommand(undefined), /non-empty string/);
  });

  await test('rejects numeric script name', () => {
    assert.throws(() => pm.getRunCommand(123), /non-empty string/);
  });

  await test('rejects boolean script name', () => {
    assert.throws(() => pm.getRunCommand(true), /non-empty string/);
  });
  describe('getExecCommand (non-string binary)');

  await test('rejects undefined binary name', () => {
    assert.throws(() => pm.getExecCommand(undefined), /non-empty string/);
  });

  await test('rejects numeric binary name', () => {
    assert.throws(() => pm.getExecCommand(42), /non-empty string/);
  });
  describe('getCommandPattern (escapeRegex completeness)');

  await test('escapes all regex metacharacters in action', () => {
  ...
  await test('escapeRegex preserves alphanumeric chars', () => {
  ...
  describe('getPackageManager (global config edge cases)');

  await test('ignores global config with non-string packageManager', () => {
  ...
  describe('Round 30: getCommandPattern edge cases');

  await test('escapes pipe character in action name', () => {
  ...
  await test('known action "dev" does NOT use escapeRegex path', () => {
  ...
  describe('setProjectPackageManager (write verification, Round 31)');

  await test('setProjectPackageManager creates .claude directory if missing', () => {
  ...
  await test('setProjectPackageManager includes setAt timestamp', () => {
  ...
  describe('getExecCommand (safe argument edge cases, Round 31)');

  await test('allows colons in args (e.g. --fix:all)', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('allows at-sign in args (e.g. @latest)', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('allows equals in args (e.g. --config=path)', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('Round 34: getExecCommand non-string args');

  await test('getExecCommand with args=0 produces command without extra args', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('getExecCommand with args=false produces command without extra args', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  await test('getExecCommand with args=null produces command without extra args', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('Round 34: detectFromPackageJson with non-string packageManager');

  await test('detectFromPackageJson handles array packageManager field gracefully', () => {
  ...
  await test('detectFromPackageJson handles numeric packageManager field gracefully', () => {
    const tmpDir = createTestDir();
    try {
  ...
  describe('Round 48: detectFromPackageJson (version format edge cases)');

  await test('returns null for packageManager with non-@ separator', () => {
    const testDir = createTestDir();
    try {
      fs.writeFileSync(path.join(testDir, 'package.json'), JSON.stringify({ name: 'test', packageManager: 'pnpm+8.6.0' }));
      const result = pm.detectFromPackageJson(testDir);
  ...
  await test('extracts package manager from caret version like yarn@^4.0.0', () => {
    const testDir = createTestDir();
    try {
  ...
  await test('getPackageManager falls through corrupted global config to npm default', () => {
    const tmpDir = createTestDir();
    ...
    try {
      ...
      delete require.cache[require.resolve('../../src/lib/package-manager')];
      delete require.cache[require.resolve('../../src/lib/utils')];
      const freshPM = require('../../src/lib/package-manager');
  ...
  describe('Round 69: getPackageManager (global-config success)');

  await test('getPackageManager returns source global-config when valid global config exists', () => {
    const tmpDir = createTestDir();
    const projDir = path.join(tmpDir, 'proj');
    fs.mkdirSync(projDir, { recursive: true });
  ...
  describe('Round 71: setPreferredPackageManager (save failure)');

  await test('setPreferredPackageManager throws wrapped error when save fails', () => {
  ...
  describe('Round 72: setProjectPackageManager (save failure)');

  await test('setProjectPackageManager throws wrapped error when write fails', () => {
    if (process.platform === 'win32' || process.getuid?.() === 0) {
      console.log(' (skipped — chmod ineffective on Windows/root)');
      return;
    }
  ...
  describe('Round 80: getExecCommand (truthy non-string args)');

  await test('getExecCommand with args=42 (truthy number) appends stringified value', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('Round 86: detectFromPackageJson (empty package.json)');

  await test('detectFromPackageJson returns null for empty (0-byte) package.json', () => {
  ...
  describe('Round 91: getCommandPattern (empty action)');

  await test('getCommandPattern with empty string returns valid regex pattern', () => {
  ...
  describe('Round 91: detectFromPackageJson (whitespace-only packageManager)');

  await test('detectFromPackageJson returns null for whitespace-only packageManager field', () => {
  ...
  describe('Round 92: detectFromPackageJson (empty string packageManager)');

  await test('detectFromPackageJson returns null for empty string packageManager field', () => {
    ...
    const testDir = createTestDir();
    fs.writeFileSync(path.join(testDir, 'package.json'), JSON.stringify({ name: 'test', packageManager: '' }));
    const result = pm.detectFromPackageJson(testDir);
  ...
  describe('Round 94: detectFromPackageJson (scoped package name @scope/pkg@version)');

  await test('detectFromPackageJson returns null for scoped package name (@scope/pkg@version)', () => {
    ...
    const testDir = createTestDir();
    fs.writeFileSync(path.join(testDir, 'package.json'), JSON.stringify({ name: 'test', packageManager: '@pnpm/exe@8.0.0' }));
    const result = pm.detectFromPackageJson(testDir);
  ...
  describe('Round 94: getPackageManager (empty string CLAUDE_PACKAGE_MANAGER env var)');

  await test('getPackageManager skips empty string CLAUDE_PACKAGE_MANAGER (falsy short-circuit)', () => {
  ...
  describe('Round 104: detectFromLockFile (null projectDir — throws TypeError)');

  await test('detectFromLockFile(null) throws TypeError (path.join rejects null)', () => {
    ...
    assert.throws(() => pm.detectFromLockFile(null), { name: 'TypeError' }, 'path.join(null, ...) should throw TypeError (no input validation in detectFromLockFile)');
  });

  // ── Round 105: getExecCommand with object args (bypasses SAFE_ARGS_REGEX, coerced to [object Object]) ──
  describe('Round 105: getExecCommand (object args — typeof bypass coerces to [object Object])');

  await test('getExecCommand with args={} bypasses SAFE_ARGS validation and coerces to "[object Object]"', () => {
    ...
    assert.throws(() => pm.getExecCommand('prettier', '[object Object]'), /unsafe characters/, 'Same string as explicit string arg is correctly rejected by SAFE_ARGS_REGEX');
  });

  // ── Round 109: getExecCommand with ../ path traversal in binary — SAFE_NAME_REGEX allows it ──
  describe('Round 109: getExecCommand (path traversal in binary — SAFE_NAME_REGEX permits ../ in binary name)');

  await test('getExecCommand accepts ../../../etc/passwd as binary because SAFE_NAME_REGEX allows ../', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('Round 108: getRunCommand (path traversal — SAFE_NAME_REGEX permits ../ via allowed / and . chars)');

  await test('getRunCommand accepts @scope/../../evil because SAFE_NAME_REGEX allows ../', () => {
    const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
    try {
      process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  describe('Round 111: getExecCommand (newline in args)');

  await test('getExecCommand accepts newline in args because SAFE_ARGS_REGEX includes newline', () => {
