...
function runTests() {
  ...
  try {
    ...
    if (test('returns null for non-existent directory', () => {
      const result = readManifest(path.join(tmpDir, 'nonexistent'));
      assert.strictEqual(result, null);
    })) passed++; else failed++;

    if (test('returns null for corrupted JSON', () => {
    ...
    if (test('returns null for JSON without required fields', () => {
    ...
    if (test('writes manifest to directory', () => {
    ...
    if (test('creates parent directories if needed', () => {
    ...
    if (test('creates manifest with correct structure', () => {
    ...
    if (test('does not share references with input', () => {
    ...
    if (test('preserves installedAt, updates updatedAt', () => {
    ...
    if (test('merges languages (union)', () => {
      const original = createManifest('1.0.0', ['typescript'], sampleArtifacts());
      const updated = updateManifest(original, '1.1.0', ['golang'], sampleArtifacts());
    ...
    if (test('does not mutate original manifest', () => {
      const original = createManifest('1.0.0', ['typescript'], sampleArtifacts());
    ...
    if (test('returns false for null manifest', () => {
    ...
    if (test('returns false for unmanaged file', () => {
      const manifest = createManifest('1.0.0', ['ts'], sampleArtifacts());
    ...
    if (test('returns true for managed rule', () => {
      const manifest = createManifest('1.0.0', ['ts'], sampleArtifacts());
      assert.strictEqual(isEccManagedRule(manifest, 'common', 'coding-style.md'), true);
    })) passed++; else failed++;

    if (test('returns false for unknown group', () => {
      const manifest = createManifest('1.0.0', ['ts'], sampleArtifacts());
    ...
    if (test('computes diff correctly', () => {
      const diff = diffFileList(
        ['a.md', 'b.md', 'c.md'],
        ['b.md', 'c.md', 'd.md'],
      );
    ...
    if (test('returns .ecc-manifest.json', () => {
