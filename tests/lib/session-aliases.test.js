...
const aliases = require('../../src/lib/session-aliases');
...
async function runTests() {
  describe('Testing session-aliases.js');
  ...
  await test('returns default structure when no file exists', () => {
      resetAliases();
  ...
  await test('returns default structure for corrupted JSON', () => {
      const aliasesPath = aliases.getAliasesPath();
  ...
  await test('returns default structure for invalid structure', () => {
      const aliasesPath = aliases.getAliasesPath();
  ...
  await test('creates a new alias', () => {
      resetAliases();
  ...
  await test('rejects empty session path', () => {
      const result = aliases.setAlias('valid-name', '');
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('empty'));
    });

  await test('accepts underscores and dashes in alias', () => {
      resetAliases();
  ...
  await test('resolves existing alias', () => {
      resetAliases();
  ...
  await test('returns null for non-existent alias', () => {
  ...
  await test('returns null for invalid alias characters', () => {
  ...
  await test('lists all aliases sorted by recency', () => {
      resetAliases();
  ...
  await test('filters aliases by search string', () => {
      const list = aliases.listAliases({ search: 'old' });
      assert.strictEqual(list.length, 1);
      assert.strictEqual(list[0].name, 'old-one');
    });

  await test('limits number of results', () => {
      const list = aliases.listAliases({ limit: 1 });
      assert.strictEqual(list.length, 1);
    });

  await test('returns empty array when no aliases exist', () => {
      resetAliases();
      const list = aliases.listAliases();
  ...
  await test('deletes existing alias', () => {
      resetAliases();
  ...
  await test('returns error for non-existent alias', () => {
  ...
  await test('renames existing alias', () => {
      resetAliases();
  ...
  await test('rejects rename to existing alias', () => {
      resetAliases();
  ...
  await test('rejects rename of non-existent alias', () => {
      const result = aliases.renameAlias('nonexistent', 'new-name');
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('not found'));
    });

  await test('rejects rename to invalid characters', () => {
      resetAliases();
  ...
  await test('rejects rename to empty string', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  await test('rejects rename to reserved name', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  await test('rejects rename to name exceeding 128 chars', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  await test('updates title of existing alias', () => {
      resetAliases();
  ...
  await test('clears title with null', () => {
  ...
  await test('rejects non-string non-null title', () => {
      const result = aliases.updateAliasTitle('titled', 42);
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('string'));
    });

  await test('rejects title update for non-existent alias', () => {
  ...
  await test('resolves alias to session path', () => {
      resetAliases();
  ...
  await test('returns input as-is when not an alias', () => {
  ...
  await test('finds all aliases for a session path', () => {
      resetAliases();
  ...
  await test('returns empty array for session with no aliases', () => {
  ...
  await test('removes aliases for non-existent sessions', () => {
      resetAliases();
  ...
  await test('handles all sessions existing (no cleanup needed)', () => {
      resetAliases();
  ...
  await test('rejects non-function sessionExists', () => {
      const result = aliases.cleanupAliases('not a function');
      assert.strictEqual(result.totalChecked, 0);
      assert.ok(result.error);
    });

  await test('handles sessionExists that throws an exception', () => {
      resetAliases();
  ...
  describe('listAliases (edge cases)');


  await test('handles entries with missing timestamps gracefully', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  await test('search matches title in addition to name', () => {
      resetAliases();
  ...
  await test('limit of 0 returns empty array', () => {
      resetAliases();
  ...
  await test('search with no matches returns empty array', () => {
      resetAliases();
  ...
  // setAlias edge cases
  describe('setAlias (edge cases)');


  await test('rejects non-string session path types', () => {
      resetAliases();
      const result = aliases.setAlias('valid-name', 42);
      assert.strictEqual(result.success, false);
    });

  await test('rejects whitespace-only session path', () => {
      resetAliases();
      const result = aliases.setAlias('valid-name', '   ');
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('empty'));
  ...
  await test('preserves createdAt on update', () => {
      resetAliases();
  ...
  describe('updateAliasTitle (edge cases)');


  await test('empty string title becomes null', () => {
      resetAliases();
  ...
  describe('saveAliases (atomic write)');


  await test('persists data across load/save cycles', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  await test('updates metadata on save', () => {
      resetAliases();
  ...
  describe('cleanupAliases (edge cases)');


  await test('returns correct totalChecked when all removed', () => {
      resetAliases();
  ...
  await test('cleanupAliases returns success:true when aliases removed', () => {
      resetAliases();
  ...
  await test('cleanupAliases returns success:true when no cleanup needed', () => {
      resetAliases();
      aliases.setAlias('alive', '/sessions/alive');
      const result = aliases.cleanupAliases(() => true);
      assert.strictEqual(result.success, true);
  ...
  await test('cleanupAliases with empty aliases file does nothing', () => {
      resetAliases();
      const result = aliases.cleanupAliases(() => true);
      assert.strictEqual(result.success, true);
      assert.strictEqual(result.removed, 0);
  ...
  await test('cleanupAliases preserves aliases where sessionExists returns true', () => {
      resetAliases();
  ...
  describe('renameAlias (edge cases)');


  await test('rename preserves session path and title', () => {
      resetAliases();
  ...
  await test('rename preserves original createdAt timestamp', () => {
      resetAliases();
  ...
  describe('getAliasesForSession (edge cases)');


  await test('does not match partial session paths', () => {
      resetAliases();
  ...
  // ── Round 26 tests ──
  describe('setAlias (reserved names case sensitivity)');


  await test('rejects uppercase reserved name LIST', () => {
      resetAliases();
  ...
  await test('rejects mixed-case reserved name Help', () => {
      resetAliases();
  ...
  await test('rejects mixed-case reserved name Set', () => {
      resetAliases();
      const result = aliases.setAlias('Set', '/path');
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('reserved'));
    });
  describe('listAliases (negative limit)');


  await test('negative limit does not truncate results', () => {
      resetAliases();
  ...
  describe('setAlias (undefined title)');


  await test('undefined title becomes null (same as explicit null)', () => {
      resetAliases();
  ...
  describe('saveAliases (failure paths, Round 31)');


  await test('saveAliases returns false for invalid data (non-serializable)', () => {
  ...
  await test('saveAliases handles writing to read-only directory gracefully', () => {
  ...
  await test('loadAliases returns fresh structure for missing file', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  describe('renameAlias rollback (Round 33)');


  await test('renameAlias with circular data triggers rollback path', () => {
  ...
  await test('renameAlias returns rolled-back error message on save failure', () => {
  ...
  await test('renameAlias rollback preserves original alias data on naming conflict', () => {
      resetAliases();
  ...
  describe('saveAliases backup/restore (Round 33)');


  await test('saveAliases creates backup before write and removes on success', () => {
      resetAliases();
  ...
  await test('saveAliases with non-serializable data returns false and preserves existing file', () => {
      resetAliases();
      ...
      assert.ok(contentAfter.includes('before-fail'), 'Original aliases data should be preserved after failed save');
    });

  // ── Round 39: atomic overwrite on Unix (no unlink before rename) ──
  describe('Round 39: atomic overwrite');


  await test('saveAliases overwrites existing file atomically', () => {
  ...
  describe('Round 48: rapid sequential saves');


  await test('rapid sequential setAlias calls maintain data integrity', () => {
      resetAliases();
      for (let i = 0; i < 5; i++) {
  ...
  describe('Round 56: Windows platform atomic write path');


  await test('Windows platform mock: unlinks existing file before rename', () => {
      resetAliases();
      ...
      try {
        ...
        assert.strictEqual(data.aliases['win-updated'].sessionPath, '2026-02-01-def456-session.tmp', 'Session path should match');
  ...
  describe('Round 64: loadAliases version/metadata backfill');


  await test('loadAliases backfills missing version and metadata fields', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
  ...
  describe('Round 67: loadAliases (empty 0-byte file)');


  await test('loadAliases returns default structure for empty (0-byte) file', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
  ...
  describe('Round 67: resolveSessionAlias (null/falsy input)');


  await test('resolveSessionAlias returns null when given null input', () => {
      resetAliases();
      const result = aliases.resolveSessionAlias(null);
      assert.strictEqual(result, null, 'Should return null for null input');
    });
  describe('Round 67: loadAliases (metadata-only backfill, version present)');


  await test('loadAliases backfills only metadata when version already present', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
  ...
  describe('updateAliasTitle save failure (Round 70)');


  await test('updateAliasTitle returns failure when saveAliases fails (read-only dir)', () => {
      ...
      try {
        ...
        delete require.cache[require.resolve('../../src/lib/session-aliases')];
        delete require.cache[require.resolve('../../src/lib/utils')];
        const freshAliases = require('../../src/lib/session-aliases');
        ...
        assert.ok(result.error.includes('Failed to update alias title'), `Should return save failure error, got: ${result.error}`);
      } finally {
        try {
  ...
  describe('Round 72: deleteAlias (save failure)');


  await test('deleteAlias returns failure when saveAliases fails (read-only dir)', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(result.error.includes('Failed to delete alias'), `Should return delete failure error, got: ${result.error}`);
      } finally {
        try {
          fs.chmodSync(isoClaudeDir, 0o755);
        } catch {
  ...
  describe('Round 73: cleanupAliases (save failure)');


  await test('cleanupAliases returns failure when saveAliases fails after removing aliases', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(result.error.includes('Failed to save after cleanup'), `Should return cleanup save failure error, got: ${result.error}`);
  ...
  describe('Round 73: setAlias (save failure)');


  await test('setAlias returns failure when saveAliases fails', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(result.error.includes('Failed to save alias'), `Should return save failure error, got: ${result.error}`);
      } finally {
        try {
          fs.chmodSync(isoClaudeDir, 0o755);
        } catch {
  ...
  describe('Round 84: listAliases (NaN date fallback in sort comparator)');


  await test('listAliases sorts entries with invalid/missing dates to the end via || 0 fallback', () => {
      ...
      assert.ok(
        (list[1].name === 'nan-alias' || list[1].name === 'missing-alias') && (list[2].name === 'nan-alias' || list[2].name === 'missing-alias'),
        'Entries with invalid/missing dates should sort to the end'
      );
    });

  // ── Round 86: loadAliases with truthy non-object aliases field ──
  describe('Round 86: loadAliases (truthy non-object aliases field)');


  await test('loadAliases resets to defaults when aliases field is a string (typeof !== object)', () => {
  ...
  describe('Round 90: saveAliases (backup restore double failure)');


  await test('saveAliases triggers inner restoreErr catch when both save and restore fail', () => {
  ...
  describe('Round 95: renameAlias (self-rename same name)');


  await test('renameAlias returns "already exists" error when renaming alias to itself', () => {
      resetAliases();
      ...
      assert.ok(result.error.includes('already exists'), 'Error should indicate alias already exists (line 333-334 check)');
  ...
  describe('Round 100: cleanupAliases (callback returns 0 — falsy non-boolean coercion)');

  await test('cleanupAliases removes alias when callback returns 0 (falsy coercion: !0 === true)', () => {
      resetAliases();
      ...
      assert.strictEqual(result.removed, 1, 'Alias should be removed because !0 === true (JavaScript falsy coercion)');
  ...
  describe('Round 102: setAlias (title=0 — falsy coercion silently converts to null)');

  await test('setAlias with title=0 stores null (0 || null === null due to JavaScript falsy coercion)', () => {
      ...
      assert.strictEqual(result.success, true, 'setAlias should succeed (0 is valid as a truthy check bypass)');
      assert.strictEqual(result.title, null, 'Title should be null because 0 || null === null (falsy coercion)');
      const resolved = aliases.resolveAlias('zero-title');
      assert.strictEqual(resolved.title, null, 'Persisted title should be null after round-trip through saveAliases/loadAliases');
    });

  // ── Round 103: loadAliases with array aliases in JSON (typeof [] === 'object' bypass) ──
  describe('Round 103: loadAliases (array aliases — typeof bypass)');

  await test('loadAliases accepts array aliases because typeof [] === "object" passes validation', () => {
      ...
      assert.ok(Array.isArray(data.aliases), 'data.aliases should be an array (typeof [] === "object" bypasses guard)');
      ...
      assert.deepStrictEqual(keys, ['0', '1', '2'], 'Object.keys of array returns numeric string indices, not named alias keys');
    });

  // ── Round 104: resolveSessionAlias with path-traversal input (passthrough without validation) ──
  describe('Round 104: resolveSessionAlias (path-traversal input — returned unchanged)');

  await test('resolveSessionAlias returns path-traversal input as-is when alias lookup fails', () => {
      ...
      assert.strictEqual(result, traversal, 'Path-traversal input should be returned as-is (resolveAlias rejects it, fallback returns input)');
      // Also test with another invalid alias pattern
      const dotSlash = './../../secrets';
      const result2 = aliases.resolveSessionAlias(dotSlash);
      assert.strictEqual(result2, dotSlash, 'Another path-traversal pattern also returned unchanged');
    });

  // ── Round 107: setAlias with whitespace-only title (not trimmed unlike sessionPath) ──
  describe('Round 107: setAlias (whitespace-only title — truthy string stored as-is, unlike sessionPath which is trim-checked)');

  await test('setAlias stores whitespace-only title as-is (no trim validation, unlike sessionPath)', () => {
      resetAliases();
      // sessionPath with whitespace is rejected (line 195: sessionPath.trim().length === 0)
      const pathResult = aliases.setAlias('ws-path', '   ');
      assert.strictEqual(pathResult.success, false, 'Whitespace-only sessionPath is rejected by trim check');
      ...
      assert.strictEqual(titleResult.success, true, 'Whitespace-only title is accepted (no trim check on title)');
      assert.strictEqual(titleResult.title, '   ', 'Title stored as whitespace string (truthy, so title || null returns the whitespace)');
      // Verify persisted correctly
      const loaded = aliases.loadAliases();
      assert.strictEqual(loaded.aliases['ws-title'].title, '   ', 'Whitespace title persists in JSON as-is');
    });

  // ── Round 111: setAlias with exactly 128-character alias — off-by-one boundary ──
  describe('Round 111: setAlias (128-char alias — exact boundary of > 128 check)');

  await test('setAlias accepts alias of exactly 128 characters (128 is NOT > 128)', () => {
      ...
      assert.strictEqual(result.success, true, '128-char alias should be accepted (128 is NOT > 128)');
      assert.strictEqual(result.isNew, true);
  ...
  describe('Round 112: resolveAlias (Unicode rejection)');

  await test('resolveAlias returns null for alias names containing Unicode characters', () => {
      resetAliases();
      ...
      assert.strictEqual(accentedResult, null, 'Accented character "é" should be rejected by [a-zA-Z0-9_-]');
      ...
      assert.strictEqual(cyrillicResult, null, 'Cyrillic homoglyph "е" (U+0435) should be rejected even though it looks like "e"');
    });

  // ── Round 114: listAliases with non-string search (number) — TypeError on toLowerCase ──
  describe('Round 114: listAliases (non-string search — number triggers TypeError)');

  await test('listAliases throws TypeError when search option is a number (no toLowerCase method)', () => {
      resetAliases();
  ...
  describe('Round 115: updateAliasTitle (empty string title — stored null, returned "")');

  await test('updateAliasTitle with empty string stores null but returns empty string (|| coercion mismatch)', () => {
      resetAliases();
      ...
      assert.strictEqual(after.title, null, 'Stored title should be null because "" || null evaluates to null');
  ...
  describe('Round 116: loadAliases (extra unknown JSON fields — preserved by loose validation)');

  await test('loadAliases preserves extra unknown fields because only aliases key is validated', () => {
      resetAliases();
      ...
      assert.strictEqual(loaded.customField, 'extra data', 'Extra string field should be preserved');
      assert.deepStrictEqual(loaded.debugInfo, { level: 3, verbose: true }, 'Extra object field should be preserved');
      assert.deepStrictEqual(loaded.tags, ['important', 'test'], 'Extra array field should be preserved');
      ...
      assert.strictEqual(reloaded.customField, 'extra data', 'Extra field should survive save/load round-trip');
    });

  // ── Round 118: renameAlias to the same name — "already exists" because self-check ──
  describe('Round 118: renameAlias (same name — "already exists" because data.aliases[newAlias] is truthy)');

  await test('renameAlias to the same name returns "already exists" error (no self-rename short-circuit)', () => {
      resetAliases();
      ...
      assert.ok(result.error.includes('already exists'), 'Error should say "already exists" (not "same name" or a no-op success)');
  ...
  describe('Round 118: setAlias (reserved names — case-insensitive rejection)');

  await test('setAlias rejects all reserved names case-insensitively (list, help, remove, delete, create, set)', () => {
      resetAliases();
      ...
      for (const name of reserved) {
        ...
        assert.ok(result.error.includes('reserved'), `Error for '${name}' should mention "reserved"`);
      }

      // Case-insensitive: uppercase variants also rejected
      const upperResult = aliases.setAlias('LIST', '/path/to/session');
      assert.strictEqual(upperResult.success, false, '"LIST" (uppercase) should be rejected (toLowerCase check)');
  ...
  describe('Round 119: renameAlias (reserved newAlias name — parallel check to setAlias)');

  await test('renameAlias rejects reserved names for newAlias (same reserved list as setAlias)', () => {
      resetAliases();
  ...
  describe('Round 120: setAlias (max alias length boundary — 128 ok, 129 rejected)');

  await test('setAlias accepts exactly 128-char alias name but rejects 129 chars (> 128 boundary)', () => {
      resetAliases();
      ...
      assert.strictEqual(result128.success, true, '128-char alias should be accepted (128 > 128 is false)');

      // 129 characters — just over limit
      const name129 = 'a'.repeat(129);
      const result129 = aliases.setAlias(name129, '/path/to/session');
      assert.strictEqual(result129.success, false, '129-char alias should be rejected (129 > 128 is true)');
  ...
  describe('Round 121: setAlias (sessionPath validation — null, empty, whitespace, non-string)');

  await test('setAlias rejects invalid sessionPath: null, empty, whitespace-only, and non-string types', () => {
      resetAliases();
  ...
  describe('Round 122: listAliases (limit edge cases — 0/negative/NaN are falsy, return all)');

  await test('listAliases limit=0 returns all aliases because 0 is falsy in JS (no slicing)', () => {
      resetAliases();
      aliases.setAlias('alias-a', '/path/a');
      aliases.setAlias('alias-b', '/path/b');
      ...
      assert.strictEqual(zeroResult.length, 3, 'limit=0 should return ALL aliases (0 is falsy in JS)');

      // limit=-1: -1 is truthy but -1 > 0 is false → no slicing → ALL returned
      const negResult = aliases.listAliases({ limit: -1 });
      assert.strictEqual(negResult.length, 3, 'limit=-1 should return ALL aliases (-1 > 0 is false)');
  ...
  describe('Round 125: loadAliases (__proto__ key in JSON — safe, no prototype pollution)');

  await test('loadAliases with __proto__ alias key does not pollute Object prototype', () => {
  ...
  "metadata": { "totalCount": 2, "lastUpdated": "${now}" }
      ...
      const freshObj = {};
      assert.strictEqual(freshObj.sessionPath, undefined, 'Object.prototype should NOT have sessionPath (no pollution)');
      assert.strictEqual(freshObj.title, undefined, 'Object.prototype should NOT have title (no pollution)');

      // The __proto__ key IS accessible as a normal property
      assert.ok(data.aliases['__proto__'], '__proto__ key exists as normal property in parsed aliases');
      assert.strictEqual(data.aliases['__proto__'].sessionPath, '/evil/path', '__proto__ alias data is accessible normally');
      ...
      if (resolved) {
        assert.strictEqual(resolved.sessionPath, '/evil/path', 'resolveAlias can access __proto__ alias (regex allows underscores)');
      }

      // Object.keys should enumerate __proto__ from JSON.parse
      const keys = Object.keys(data.aliases);
      assert.ok(keys.includes('__proto__'), 'Object.keys includes __proto__ from JSON.parse (normal property)');
