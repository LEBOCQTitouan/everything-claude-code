...
function runTests() {
  ...
  try {
    if (test('returns false for non-git directory', () => {
  ...
  try {
    if (test('returns true for git repo', () => {
      initGitRepo(tmpDir2);
  ...
  if (test('contains expected entries', () => {
  ...
  if (test('each entry has pattern and comment', () => {
  ...
  try {
    if (test('creates .gitignore with ECC entries in git repo', () => {
      initGitRepo(tmpDir4);
  ...
  try {
    if (test('appends to existing .gitignore', () => {
      initGitRepo(tmpDir5);
  ...
  try {
    if (test('does not duplicate existing entries', () => {
      initGitRepo(tmpDir6);
  ...
  try {
    if (test('is idempotent (running twice produces same result)', () => {
      initGitRepo(tmpDir7);
  ...
  try {
    if (test('supports custom entries', () => {
      initGitRepo(tmpDir8);
      const customEntries = [
        { pattern: 'custom/path', comment: 'Custom path' },
      ];
  ...
  try {
    if (test('returns empty for non-git repo', () => {
  ...
  try {
    if (test('detects tracked ECC files', () => {
