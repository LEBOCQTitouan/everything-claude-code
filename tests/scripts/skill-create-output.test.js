...
// Import the module
const { SkillCreateOutput } = require('../../src/skill-create-output');
...
async function runTests() {
  describe('Testing skill-create-output.js');

  // Constructor tests
  describe('SkillCreateOutput constructor');

  await test('creates instance with repo name', () => {
  ...
  await test('accepts custom width option', () => {
  ...
  await test('outputs header with repo name', () => {
  ...
  await test('header handles long repo names without crash', () => {
  ...
  await test('displays analysis data', () => {
  ...
  await test('displays patterns with confidence bars', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() =>
      output.patterns([
  ...
  await test('handles patterns with missing confidence', () => {
    const output = new SkillCreateOutput('repo');
    // Should default to 0.8 confidence
    const logs = captureLog(() => output.patterns([{ name: 'No Confidence', trigger: 'always', evidence: 'evidence' }]));
    const combined = logs.join('\n');
  ...
  await test('displays instincts in a box', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() =>
      output.instincts([
  ...
  await test('displays file paths', () => {
    const output = new SkillCreateOutput('repo');
  ...
  await test('displays next steps with commands', () => {
    const output = new SkillCreateOutput('repo');
  ...
  await test('displays footer with attribution', () => {
    const output = new SkillCreateOutput('repo');
  ...
  await test('does not crash with confidence > 1.0 (percent > 100)', () => {
    const output = new SkillCreateOutput('repo');
    // confidence 1.5 => percent 150 — previously crashed with RangeError
    const logs = captureLog(() => output.patterns([{ name: 'Overconfident', trigger: 'always', confidence: 1.5, evidence: 'too much' }]));
    const combined = stripAnsi(logs.join('\n'));
    assert.ok(combined.includes('150%'), 'Should show 150%');
  });

  await test('renders 0% confidence bar without crash', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.patterns([{ name: 'Zero Confidence', trigger: 'never', confidence: 0.0, evidence: 'none' }]));
    const combined = stripAnsi(logs.join('\n'));
    assert.ok(combined.includes('0%'), 'Should show 0%');
  });

  await test('renders 100% confidence bar without crash', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.patterns([{ name: 'Perfect', trigger: 'always', confidence: 1.0, evidence: 'certain' }]));
    const combined = stripAnsi(logs.join('\n'));
  ...
  await test('patterns() with empty array produces header but no entries', () => {
    const output = new SkillCreateOutput('repo');
  ...
  await test('instincts() with empty array produces box but no entries', () => {
    const output = new SkillCreateOutput('repo');
  ...
  describe('box() crash prevention');

  await test('box does not crash on title longer than width', () => {
    ...
    const logs = captureLog(() => output.instincts([{ name: 'a-very-long-instinct-name', confidence: 0.9 }]));
    assert.ok(logs.length > 0, 'Should produce output without crash');
  });

  await test('analysisResults does not crash with very narrow width', () => {
  ...
  await test('top, middle, and bottom lines have equal visual width', () => {
    ...
    if (boxDrawn.length >= 3) {
      ...
      widths.forEach((w, i) => {
        assert.strictEqual(w, firstWidth, `Line ${i} width ${w} should match first line width ${firstWidth}`);
      });
    }
  });

  // ── Round 27: box and progressBar edge cases ──
  describe('box() content overflow');

  await test('box does not crash when content line exceeds width', () => {
    const output = new SkillCreateOutput('repo', { width: 30 });
    // Force a very long instinct name that exceeds width
    const logs = captureLog(() => output.instincts([{ name: 'this-is-an-extremely-long-instinct-name-that-clearly-exceeds-width', confidence: 0.9 }]));
    // Math.max(0, padding) should prevent RangeError
    assert.ok(logs.length > 0, 'Should produce output without RangeError');
  });

  await test('patterns renders negative confidence without crash', () => {
    const output = new SkillCreateOutput('repo');
    // confidence -0.1 => percent -10 — Math.max(0, ...) should clamp filled to 0
    const logs = captureLog(() => output.patterns([{ name: 'Negative', trigger: 'never', confidence: -0.1, evidence: 'impossible' }]));
    const combined = stripAnsi(logs.join('\n'));
    assert.ok(combined.includes('-10%'), 'Should show -10%');
  });

  await test('header does not crash with very long repo name', () => {
  ...
  await test('stripAnsi handles nested ANSI codes with multi-digit params', () => {
  ...
  await test('footer produces output', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.footer());
    const combined = stripAnsi(logs.join('\n'));
  ...
  describe('header() width alignment (Round 34)');

  await test('header subtitle line matches border width', () => {
    const output = new SkillCreateOutput('test-repo');
    const logs = captureLog(() => output.header());
    ...
    assert.strictEqual(subtitleLine.length, borderLine.length, `Subtitle width (${subtitleLine.length}) should match border width (${borderLine.length})`);
  });

  await test('header all lines have consistent width for short repo name', () => {
  ...
  await test('header subtitle has correct content area width of 64 chars', () => {
    ...
    assert.strictEqual(subtitleLine.length, 66, `Total subtitle line width should be 66, got ${subtitleLine.length}`);
  });

  await test('header subtitle line does not truncate with medium-length repo name', () => {
    ...
    assert.strictEqual(subtitleLine.length, 66, `Subtitle line should be 66 chars, got ${subtitleLine.length}`);
  });

  // ── Round 35: box() width accuracy ──
  describe('box() width accuracy (Round 35)');

  await test('box lines in instincts() match the default box width of 60', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.instincts([{ name: 'test-instinct', confidence: 0.85 }]));
    const combined = logs.join('\n');
    const boxLines = combined.split('\n').filter(l => {
      const s = stripAnsi(l).trim();
      return s.startsWith('\u256D') || s.startsWith('\u2502') || s.startsWith('\u2570');
  ...
  await test('box lines with custom width match the requested width', () => {
    const output = new SkillCreateOutput('repo', { width: 40 });
    ...
    boxLines.forEach((l, i) => {
      const w = stripAnsi(l).length;
      assert.strictEqual(w, firstWidth, `Box line ${i} width ${w} should match first line ${firstWidth}`);
    });
  });

  await test('analysisResults box lines are all 60 chars wide', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() =>
      output.analysisResults({
        commits: 50,
  ...
  await test('nextSteps box lines are all 60 chars wide', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.nextSteps());
    const combined = logs.join('\n');
    const boxLines = combined.split('\n').filter(l => {
  ...
  describe('analysisResults zero values (Round 54)');

  await test('analysisResults handles zero values for all data fields', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() =>
      output.analysisResults({
        commits: 0,
  ...
  describe('demo export (Round 68)');

  await test('module exports demo function alongside SkillCreateOutput', () => {
    const mod = require('../../src/skill-create-output');
  ...
  describe('Round 85: patterns() confidence=0 nullish coalescing');

  await test('patterns() with confidence=0 shows 0%, not 80% (nullish coalescing fix)', () => {
    const output = new SkillCreateOutput('repo');
    const logs = captureLog(() => output.patterns([{ name: 'Zero Confidence', trigger: 'never', confidence: 0, evidence: 'none' }]));
    const combined = stripAnsi(logs.join('\n'));
    ...
    assert.ok(!combined.includes('80%'), 'Should NOT show 80% — confidence=0 is explicitly provided, not missing');
  });

  // ── Round 87: analyzePhase() async method (untested) ──
  describe('Round 87: analyzePhase() async method');

  await test('analyzePhase completes without error and writes to stdout', () => {
    const output = new SkillCreateOutput('test-repo');
    ...
    try {
      ...
      assert.ok(promise && typeof promise.then === 'function', 'analyzePhase should return a Promise');
    } finally {
