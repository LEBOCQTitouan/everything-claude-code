...
async function runTests() {
  describe('Testing claw.js');

  // ── Storage tests (6) ──────────────────────────────────────────────────

  describe('Storage');

  await test('getClawDir() returns path ending in .claude/claw', () => {
    const dir = getClawDir();
    assert.ok(dir.endsWith(path.join('.claude', 'claw')), `Expected path ending in .claude/claw, got: ${dir}`);
  });

  await test('getSessionPath("foo") returns correct .md path', () => {
    const p = getSessionPath('foo');
    assert.ok(p.endsWith(path.join('.claude', 'claw', 'foo.md')), `Expected path ending in .claude/claw/foo.md, got: ${p}`);
  });

  await test('listSessions() returns empty array for empty dir', () => {
  ...
  await test('listSessions() finds .md files and strips extension', () => {
    const tmpDir = makeTmpDir();
    try {
  ...
  await test('loadHistory() returns "" for non-existent file', () => {
    const result = loadHistory('/tmp/claw-test-nonexistent-' + Date.now() + '.md');
    assert.strictEqual(result, '');
  });

  await test('appendTurn() writes correct markdown format', () => {
    const tmpDir = makeTmpDir();
    const filePath = path.join(tmpDir, 'test.md');
    try {
      appendTurn(filePath, 'User', 'Hello world', '2025-01-15T10:00:00.000Z');
      const content = fs.readFileSync(filePath, 'utf8');
      assert.ok(content.includes('### [2025-01-15T10:00:00.000Z] User'), 'Should include timestamp and role header');
  ...
  await test('loadECCContext() returns "" when no skills specified', () => {
    const result = loadECCContext('');
    assert.strictEqual(result, '');
  });

  await test('loadECCContext() skips missing skill directories gracefully', () => {
    const result = loadECCContext('nonexistent-skill-xyz');
    assert.strictEqual(result, '');
  });

  await test('loadECCContext() concatenates multiple skill files', () => {
  ...
  await test('buildPrompt() constructs correct prompt structure', () => {
  ...
  await test('askClaude() handles subprocess error gracefully', () => {
  ...
  await test('module exports all required functions', () => {
  ...
  await test('/clear truncates session file', () => {
    const tmpDir = makeTmpDir();
  ...
  await test('isValidSessionName rejects invalid characters', () => {
