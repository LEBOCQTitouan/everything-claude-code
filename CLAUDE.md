...
```bash
cargo test              # Run all Rust tests (3398 tests)
...
ecc backlog update-status BL-NNN <s> Update entry status (open|implemented|archived|promoted|in-progress)
ecc backlog migrate                  One-time migration: sync file statuses from BACKLOG.md index
...
`/project-foundation` — standalone command for bootstrapping project-level foundational documents (PRD, architecture, ADR) via guided interview with codebase-aware challenge. Not part of the spec pipeline — operates one level above feature specs.
...
- Multi-session safety (BL-156): parallel Claude Code sessions in the same repo are safe. Each ECC hook (SessionStart/PostToolUse/Stop) writes `<worktree>/.ecc-session` atomically (PID + timestamp). `ecc worktree gc` skips worktrees with fresh heartbeats AND live PIDs. Self-skip via `CLAUDE_PROJECT_DIR` prevents the session-start GC from deleting its own worktree. Override: `--force --kill-live --yes` (destructive, requires two explicit flags + confirmation). Kill switch: `ECC_WORKTREE_LIVENESS_DISABLED=1` short-circuits read+write paths back to BL-150 logic. TTL configurable via `ECC_WORKTREE_LIVENESS_TTL_SECS` (default 3600s). See ADR 0068.
...
- **tool-set** = agent/command frontmatter key referencing a named preset in `manifest/tool-manifest.yaml`; expanded to inline `tools:` at install time so Claude Code runtime is unaffected (BL-146, ADR 0060)
- **party panel** = ephemeral multi-agent round-table assembled by /party command from BMAD roles (bmad-pm, bmad-architect, bmad-dev, bmad-qa, bmad-security) + ECC specialists + project-domain agents; dispatched sequentially by party-coordinator; output persisted to docs/party/ (BL-144, ADR 0064)
- Glossary: **write-guard** = PreToolUse hook blocking writes outside worktree (exit 2); **lazy worktree** = worktree created on-demand at first write; **session merge** = automatic rebase+verify+ff-merge at session end; **fix-round budget** = max 2 fix attempts per PC before user escalation; **coverage delta** = before/after test coverage % comparison across TDD loop; **bounded context enumeration** = listing affected DDD contexts in /design output; **per-test-name inventory** = individual test function names from TDD cycles; **harness metrics** = hook success rate, phase-gate violation rate, agent recovery rate, commit atomicity score (SLO targets: 99%/5%/80%/95%); **tracer bullet** = thin vertical slice through all architectural layers, implemented first to validate architecture E2E before horizontal expansion; **coverage gate** = CI job that enforces ≥80% workspace function coverage via --fail-under-functions; blocks PR merge if threshold not met; **foundation document** = project-level PRD, architecture overview, or initial ADR produced by `/project-foundation` (distinct from feature-level specs); **codebase-analysis phase** = automated detection of tech stack, architecture patterns, and bounded contexts from existing repo structure during `/project-foundation`; **additive alignment** = AAIF alignment stance where ECC adopts core AGENTS.md fields as-is and treats ECC-specific fields (effort, skills, memory, tracking, patterns) as non-conflicting extensions (ADR 0062); **self-evaluation** = post-PC evaluation step in /implement checking AC satisfaction, regression heuristics, and spec achievability via read-only pc-evaluator subagent (conditional: fix rounds > 0, integration/e2e, wave boundary); **compass context file** = 25-35 line module orientation doc in docs/context/ with Quick Commands, Key Files, Non-Obvious Patterns, Cross-References; generated by compass-context-writer agent (BL-152); **tribal knowledge extraction** = five-question framework per module capturing configure/provide, common mods, failure patterns, hidden deps, comment knowledge; **auto-repair** = tiered automated doc drift fix: LOW/MEDIUM auto-fixed, HIGH/CRITICAL flagged for manual review
- Backward transitions (BL-129): `ecc-workflow transition <phase> --justify "reason"` rolls back to an earlier phase. Allowed backward pairs: implement→solution, solution→plan, implement→plan. Justification is mandatory (non-empty, non-whitespace). Artifacts for rolled-back phases are cleared (timestamps AND paths). Forward re-entry resumes normally with re-stamped timestamps. All transitions (forward + backward) are logged to `state.json.history[]` as `TransitionRecord`. View with `ecc workflow history [--json]`. `TransitionPolicy::default_forward()` = forward-only (used by legacy `resolve_transition`); `TransitionPolicy::with_backward()` = full (used by `resolve_transition_with_justification`). (ADR 0064, ADR 0065)
- Slash-command templates MUST NOT use `$ARGUMENTS` in `!`-prefix shell-eval lines (ADR 0066, 2026-04-17). zsh substitutes `$ARGUMENTS` literally before tokenization, so metacharacters in user input corrupt parsing. Use the Bash tool with env-var + stdin pattern: `env FEATURE='<text>' sh -c 'printf %s "$FEATURE" | ecc-workflow init dev --feature-stdin'`. Enforcement: `ecc validate commands` rejects `^[[:space:]]*!.*\$ARGUMENTS` patterns. `ecc-workflow init` / `worktree-name` accept `--feature-stdin` (64KB cap, UTF-8 only, rejects TTY, strips single trailing LF) and the `ecc workflow` delegator mirrors it.
- Cartography write-time filter excludes `.claude/workflow/`, `.claude/cartography/`, `.claude/worktrees/`, `docs/specs/`, `docs/backlog/`, `docs/cartography/`, and exact `Cargo.lock`. Sessions touching only these produce no `pending-delta-*.json`. Escape hatches: `ECC_CARTOGRAPHY_DEDUPE=0` disables hash dedupe; `ECC_DAILY_SUMMARY_NOISE_FILTER=0` disables daily filter.
- Memory prune fires automatically when a backlog entry transitions to `implemented`: matching `project_bl<N>_*.md` files and tagged `MemoryStore` entries are trashed to `<memory_root>/.trash/<date>/` (7-day retention). One-shot catch-up: `ecc memory prune --orphaned-backlogs [--apply]` (dry-run default). Recovery: `ecc memory restore --trash <YYYY-MM-DD> [--apply]`.
