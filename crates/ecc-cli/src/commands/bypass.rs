...
pub fn run(args: BypassArgs) -> anyhow::Result<()> {
    ...
    match args.action {
        BypassAction::Grant { hook, reason } => {
            ...
            let token = ecc_app::bypass_mgmt::grant(&store, &fs, &home, &hook, &reason, &session_id)?;
            println!("Bypass granted for hook '{}' in session '{}'", token.hook_id, token.session_id);
            println!("Token file: {}/.ecc/bypass-tokens/{}/{}.json",
                home.display(), session_id, hook.replace(':', "__"));
            Ok(())
        }
            ...
            if decisions.is_empty() {
                ...
                for d in &decisions {
                    println!("{:<5} {:<40} {:<10} {:<20} {}",
                        ...
                        if d.reason.len() > 30 { &d.reason[..30] } else { &d.reason });
                }
            }
            Ok(())
        }
        BypassAction::Summary => {
            ...
            for h in &summary.per_hook {
                ...
                let ratio = if total > 0 { h.accepted as f64 / total as f64 } else { 0.0 };
                println!("{:<40} {:<10} {:<10} {:.1}%", h.hook_id, h.accepted, h.refused, ratio * 100.0);
            }
            println!("\nTotal: {} accepted, {} refused", summary.total_accepted, summary.total_refused);
            Ok(())
        }
...
fn parse_duration_days(s: &str) -> anyhow::Result<u64> {
    if let Some(days_str) = s.strip_suffix('d') {
        days_str.parse::<u64>().map_err(|_| anyhow::anyhow!("Invalid duration: {s}"))
    } else {
        s.parse::<u64>().map_err(|_| anyhow::anyhow!("Invalid duration: {s}. Use format: 90d"))
    }
}
...
mod tests {
    ...
    fn bypass_parse_grant_args() {
        let cli = TestCli::try_parse_from(["test", "grant", "--hook", "pre:write:guard", "--reason", "hotfix"]);
        assert!(cli.is_ok());
    }
