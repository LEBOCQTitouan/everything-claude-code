...
- `ecc-ports`: Trait definitions only (`FileSystem`, `ShellExecutor`, `Environment`, `TerminalIO`). No implementations.
...
- `ecc-test-support`: In-memory test doubles (`InMemoryFileSystem`, `MockExecutor`, `MockEnvironment`). Used by unit and integration tests.
