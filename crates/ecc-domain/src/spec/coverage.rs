...
mod tests {
    ...
    fn ac(us: u16, sub: u16) -> AcceptanceCriterion {
        AcceptanceCriterion {
            id: AcId { us_number: us, sub_number: sub },
    ...
    fn all_acs_covered() {
        ...
        let pcs = vec![pc(1, vec![AcId { us_number: 1, sub_number: 1 }, AcId { us_number: 1, sub_number: 2 }])];
    ...
    fn uncovered_ac_reported() {
        let acs = vec![ac(1, 1), ac(1, 2)];
        let pcs = vec![pc(1, vec![AcId { us_number: 1, sub_number: 1 }])];
        let report = check_coverage(&acs, &pcs);
        assert_eq!(report.uncovered_acs.len(), 1);
        assert_eq!(report.uncovered_acs[0], AcId { us_number: 1, sub_number: 2 });
    }

    #[test]
    fn phantom_ac_detected() {
        let acs = vec![ac(1, 1)];
        let pcs = vec![pc(1, vec![AcId { us_number: 1, sub_number: 1 }, AcId { us_number: 4, sub_number: 1 }])];
        let report = check_coverage(&acs, &pcs);
    ...
    fn multiple_pcs_same_ac() {
        let acs = vec![ac(1, 1)];
        let pcs = vec![
            pc(1, vec![AcId { us_number: 1, sub_number: 1 }]),
            pc(2, vec![AcId { us_number: 1, sub_number: 1 }]),
        ];
        let report = check_coverage(&acs, &pcs);
        assert!(report.uncovered_acs.is_empty());
