...
fn dual_invocation() {
    ...
    assert_eq!(cli_value.get("status").and_then(|v| v.as_str()), Some("pass"), "CLI mode expected status 'pass'");
    ...
    assert_eq!(stdin_value.get("status").and_then(|v| v.as_str()), Some("pass"), "Stdin JSON mode expected status 'pass'");

    assert_eq!(cli_value.get("status"), stdin_value.get("status"), "CLI and stdin JSON modes must produce equivalent status");
    ...
    assert_eq!(cli_state.get("phase"), stdin_state.get("phase"), "Both modes must produce the same phase");
    assert_eq!(cli_state.get("concern"), stdin_state.get("concern"), "Both modes must produce the same concern");
    assert_eq!(cli_state.get("feature"), stdin_state.get("feature"), "Both modes must produce the same feature");
}
...
fn init_matches_shell() {
    ...
    let v: serde_json::Value = serde_json::from_str(&content).expect("state.json must be valid JSON");
    ...
    assert!(started_at.len() == 20 && started_at.ends_with('Z') && started_at.contains('T'),
    ...
    for key in &["plan", "solution", "implement", "campaign_path", "spec_path", "design_path", "tasks_path"] {
        assert_eq!(v["artifacts"][key], serde_json::Value::Null, "artifacts.{key} must be null");
    }
    assert_eq!(v["completed"], serde_json::Value::Array(vec![]), "completed must be []");
    ...
    std::fs::write(workflow_dir2.join("state.json"), serde_json::to_string_pretty(&stale_json).unwrap()).unwrap();
    ...
    assert_eq!(output2.status.code(), Some(0), "init must exit 0 even when overwriting stale state");
    ...
    let archived: Vec<_> = std::fs::read_dir(&archive_dir).unwrap().filter_map(|e| e.ok()).collect();
    assert!(!archived.is_empty(), "at least one archived state file must exist in archive/");
    assert!(!workflow_dir2.join("implement-done.md").exists(), "implement-done.md must be cleaned up during init");
    assert!(!workflow_dir2.join(".tdd-state").exists(), ".tdd-state must be cleaned up during init");

    let new_content = std::fs::read_to_string(workflow_dir2.join("state.json")).expect("new state.json must exist");
...
fn toolchain_persist() {
    ...
    let persist_output = Command::new(&bin)
        .args(["toolchain-persist", "cargo test", "cargo clippy -- -D warnings", "cargo build"])
        .env("CLAUDE_PROJECT_DIR", temp_dir.path())
        .output()
        .expect("failed to execute ecc-workflow toolchain-persist");

    assert_eq!(persist_output.status.code(), Some(0), "toolchain-persist must exit 0");

    let state_path = temp_dir.path().join(".claude/workflow/state.json");
    let content = std::fs::read_to_string(&state_path).expect("failed to read state.json after toolchain-persist");
    let value: serde_json::Value = serde_json::from_str(&content).expect("state.json must be valid JSON");
    let toolchain = value.get("toolchain").expect("missing 'toolchain' field in state.json");

    assert_eq!(toolchain.get("test").and_then(|v| v.as_str()), Some("cargo test"), "toolchain.test must be 'cargo test'");
    assert_eq!(toolchain.get("lint").and_then(|v| v.as_str()), Some("cargo clippy -- -D warnings"), "toolchain.lint must match");
    assert_eq!(toolchain.get("build").and_then(|v| v.as_str()), Some("cargo build"), "toolchain.build must be 'cargo build'");
}
