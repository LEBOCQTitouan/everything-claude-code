...
fn run_pass_condition_check(project_dir: &std::path::Path, bin: &std::path::Path) -> std::process::Output {
    Command::new(bin)
...
fn e2e_boundary_check() {
    ...
    fn run_e2e_boundary_check(project_dir: &std::path::Path, bin: &std::path::Path) -> std::process::Output {
        Command::new(bin)
    ...
    assert_eq!(out1.status.code(), Some(0), "e2e-boundary-check must exit 0 with no state.json");
    assert!(
    ...
    assert_eq!(out2.status.code(), Some(0), "e2e-boundary-check must exit 0 in plan phase");
    assert!(
    ...
    std::fs::write(workflow_dir3.join("implement-done.md"), "# Done\n\n## E2E Tests\n\n- Covered by existing suite\n").unwrap();
    let out3 = run_e2e_boundary_check(dir_done_with_section.path(), &bin);
    assert_eq!(out3.status.code(), Some(0), "e2e-boundary-check must exit 0 when E2E Tests section present");
    let stderr3 = std::str::from_utf8(&out3.stderr).unwrap_or("");
    assert!(stderr3.trim().is_empty(), "e2e-boundary-check must be silent when E2E Tests section present");
    ...
    std::fs::write(workflow_dir4.join("implement-done.md"), "# Done\n\n## Docs Updated\n\n- Some doc\n").unwrap();
    let out4 = run_e2e_boundary_check(dir_done_missing.path(), &bin);
    assert_eq!(out4.status.code(), Some(0), "e2e-boundary-check must always exit 0");
    let stderr4 = std::str::from_utf8(&out4.stderr).unwrap_or("");
    assert!(!stderr4.trim().is_empty(), "e2e-boundary-check must warn when E2E Tests section is missing");
    assert!(stderr4.to_lowercase().contains("e2e"), "warning must mention E2E, got stderr: {stderr4}");
}
...
fn pass_condition_check() {
    ...
    assert_eq!(out1.status.code(), Some(0), "scenario 1: must exit 0 for all-pass results");
    assert!(
        out1.stdout.trim_ascii().is_empty() && out1.stderr.trim_ascii().is_empty(),
    ...
    assert_eq!(out2.status.code(), Some(0), "scenario 2: must exit 0 when section missing");
    let stderr2 = std::str::from_utf8(&out2.stderr).unwrap_or("");
    assert!(!stderr2.trim().is_empty(), "scenario 2: must warn when '## Pass Condition Results' section is missing");
    assert!(stderr2.to_lowercase().contains("pass condition"), "scenario 2: warning must mention 'pass condition'");
    ...
    assert_eq!(out3.status.code(), Some(0), "scenario 3: must exit 0 even with failures");
    let stderr3 = std::str::from_utf8(&out3.stderr).unwrap_or("");
    assert!(!stderr3.trim().is_empty(), "scenario 3: must warn when ❌ failures found");
    assert!(
    ...
    assert_eq!(out4.status.code(), Some(0), "scenario 4: must exit 0 in plan phase");
    assert!(
