...
fn doc_enforcement() {
    ...
    fn write_state(dir: &std::path::Path, phase: &str) {
        ...
        let state = serde_json::json!({"phase": phase, "concern": "dev", "feature": "test-feature"});
        std::fs::write(workflow_dir.join("state.json"), serde_json::to_string(&state).unwrap()).unwrap();
    }
    ...
    assert!(stderr1.is_empty(), "scenario 1: expected no stderr warning, got: '{stderr1}'");

    // ── Scenario 2: missing "## Docs Updated" → exit 0, warning on stderr ────
    let dir2 = tempfile::tempdir().unwrap();
    write_state(dir2.path(), "done");
    write_implement_done(dir2.path(), "## Supplemental Docs\n- docs/specs/my-feature/design.md\n");
    ...
    assert!(!stderr2.is_empty(), "scenario 2: expected warning on stderr when '## Docs Updated' missing");
    let json2: serde_json::Value = serde_json::from_str(&stderr2)
        .unwrap_or_else(|e| panic!("scenario 2: stderr is not valid JSON: {e}\nstderr: {stderr2}"));
    assert_eq!(json2.get("status").and_then(|v| v.as_str()), Some("warn"), "scenario 2: expected status 'warn'");
    ...
    assert!(!stderr3.is_empty(), "scenario 3: expected warning on stderr when '## Supplemental Docs' missing");
    let json3: serde_json::Value = serde_json::from_str(&stderr3)
        .unwrap_or_else(|e| panic!("scenario 3: stderr is not valid JSON: {e}\nstderr: {stderr3}"));
    assert_eq!(json3.get("status").and_then(|v| v.as_str()), Some("warn"), "scenario 3: expected status 'warn'");

    // ── Scenario 4: state at "plan" → exit 0, no warning ─────────────────────
    let dir4 = tempfile::tempdir().unwrap();
    write_state(dir4.path(), "plan");
    let out4 = run_doc_enforcement(dir4.path(), &bin);
    assert_eq!(out4.status.code(), Some(0), "scenario 4: must exit 0 for non-done phase");
    let stderr4 = std::str::from_utf8(&out4.stderr).unwrap_or("").trim().to_string();
    assert!(stderr4.is_empty(), "scenario 4: expected no stderr for non-done phase, got: '{stderr4}'");

    // ── Scenario 5: no state.json → exit 0, silent ───────────────────────────
    let dir5 = tempfile::tempdir().unwrap();
    let out5 = run_doc_enforcement(dir5.path(), &bin);
    assert_eq!(out5.status.code(), Some(0), "scenario 5: must exit 0 when no state.json");
    let stderr5 = std::str::from_utf8(&out5.stderr).unwrap_or("").trim().to_string();
    assert!(stderr5.is_empty(), "scenario 5: expected silent output when no state.json, got: '{stderr5}'");
}
...
fn doc_level_check() {
    ...
    assert_eq!(out1.status.code(), Some(0), "doc-level-check must exit 0 with no state.json");
    assert!(
    ...
    assert_eq!(out2.status.code(), Some(0), "doc-level-check must exit 0 in plan phase");
    assert!(
    ...
    assert_eq!(out3.status.code(), Some(0), "doc-level-check must exit 0 for small CLAUDE.md");
    let stderr3 = std::str::from_utf8(&out3.stderr).unwrap_or("");
    assert!(!stderr3.to_lowercase().contains("claude.md"), "doc-level-check must not warn for small CLAUDE.md");
    ...
    assert_eq!(out4.status.code(), Some(0), "doc-level-check must always exit 0");
    let stderr4 = std::str::from_utf8(&out4.stderr).unwrap_or("");
    assert!(stderr4.to_lowercase().contains("claude.md"), "doc-level-check must warn about oversized CLAUDE.md");
}
