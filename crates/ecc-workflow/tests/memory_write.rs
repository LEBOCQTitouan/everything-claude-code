...
fn memory_write_subcommands() {
    ...
    let action_output = Command::new(&bin)
        .args(["memory-write", "action", "plan", "test feature", "success", "[]"])
    ...
    assert!(action_log_path.exists(), "docs/memory/action-log.json must exist after memory-write action");
    ...
    assert!(entry.get("session_id").is_some(), "entry missing 'session_id' field: {entry}");
    assert_eq!(entry.get("action_type").and_then(|v| v.as_str()), Some("plan"), "entry.action_type must be 'plan'");
    assert_eq!(entry.get("description").and_then(|v| v.as_str()), Some("test feature"), "entry.description must be 'test feature'");
    assert_eq!(entry.get("outcome").and_then(|v| v.as_str()), Some("success"), "entry.outcome must be 'success'");
    assert!(entry.get("artifacts").is_some(), "entry missing 'artifacts' field: {entry}");
    ...
    assert!(work_items_dir.exists(), "docs/memory/work-items/ directory must exist after memory-write work-item");

    let item_entries: Vec<_> = std::fs::read_dir(&work_items_dir)
        .expect("failed to read work-items dir")
        .filter_map(|e| e.ok())
        .collect();
    assert_eq!(item_entries.len(), 1, "must have exactly one work-item subdirectory");

    let item_dir = item_entries[0].path();
    let dir_name = item_dir.file_name().unwrap().to_string_lossy();
    assert!(dir_name.starts_with("20") && dir_name.contains('-'), "work-item dir must start with date, got: '{dir_name}'");

    let plan_file = item_dir.join("plan.md");
    assert!(plan_file.exists(), "plan.md must exist in work-item directory {:?}", item_dir);

    let plan_content = std::fs::read_to_string(&plan_file).expect("failed to read plan.md");
    assert!(plan_content.contains("# Plan:"), "plan.md must contain '# Plan:' heading");
    assert!(plan_content.contains("## Context"), "plan.md must contain '## Context' section");
    assert!(plan_content.contains("## Decisions"), "plan.md must contain '## Decisions' section");
    assert!(plan_content.contains("## Outcome"), "plan.md must contain '## Outcome' section");
    ...
    let daily_dir = home_path.join(".claude/projects").join(&project_hash).join("memory/daily");
    assert!(daily_dir.exists(), "daily memory directory must exist at {:?}", daily_dir);
    ...
    let daily_content = std::fs::read_to_string(daily_files[0].path()).expect("failed to read daily file");
    assert!(daily_content.contains("## Activity"), "daily file must contain '## Activity' section");
    assert!(daily_content.contains("## Insights"), "daily file must contain '## Insights' section");
    assert!(daily_content.contains("**plan**"), "daily file must contain '**plan**' entry");
    ...
    let memory_file = home_path.join(".claude/projects").join(&project_hash).join("memory/MEMORY.md");
    assert!(memory_file.exists(), "MEMORY.md must exist at {:?}", memory_file);

    let memory_content = std::fs::read_to_string(&memory_file).expect("failed to read MEMORY.md");
    assert!(memory_content.contains("## Daily"), "MEMORY.md must contain '## Daily' section");
    assert!(memory_content.contains("daily/"), "MEMORY.md must contain a daily/ link");
    assert!(memory_content.contains(".md)"), "MEMORY.md must contain a .md) link");
}
