...
pub mod tool_manifest_propagation {
    use super::*;

    // ── PC-055: install_expands_tool_sets_from_manifest ──────────────────────

    /// After `ecc install --force --no-interactive`, agents with `tool-set:` references
    /// should have `tools:` inline and no `tool-set:` in their content.
    #[test]
    pub fn install_expands_tool_sets_from_manifest() {
        ...
        // Verify no installed agents contain tool-set:
    ...
    #[test]
    pub fn validate_ecc_content_against_manifest() {
        let env = EccTestEnv::new();
    ...
    #[test]
    pub fn no_tool_set_in_installed_output() {
        let env = EccTestEnv::new();
        env.install(&[]).success();
    ...
    /// Two fixture representations — one with inline `tools: [Read, Grep, Glob]` and
    /// one derived from `tool-set: readonly-analyzer` — must produce SHA-256-identical
    /// `tools:` lines after expansion.
    #[test]
    pub fn install_sha256_pre_post_match() {
        ...
        let manifest = ecc_domain::config::tool_manifest::parse_tool_manifest(&manifest_str)
            .expect("parse manifest");

        // Extract preset tools and build the expanded tools line
        ...
        // SHA-256 of expanded line
        let expanded_hash = sha256_of_str(&tools_str);

        // SHA-256 of the inline representation
    ...
    #[test]
    pub fn validate_teams_byte_identical_pre_post() {
        let env = EccTestEnv::new();
